...
public sealed class WorldBootstrapDefinition
{
    ...
    // §70: where each faction pitches its camp. Empty = a single-camp world,
    // which is exactly the pre-§70 behaviour (and what the test bootstraps want).
...
public sealed class NpcBootstrap
{
    ...
    // §70: Colony (the girls) unless the world explicitly seeds an outsider.
    public Agents.Faction Faction { get; set; } = Agents.Faction.Colony;
