...
// §107: арифметика сговора. «Кто стоит рядом», «о ком речь», «согласны ли
...
public static class GroupHuntMath
{
        ...
        foreach (var other in world.Entities.Npcs.Values)
        {
            if (other.Health <= 0f ||
                ...
                HexSpatialMath.HexDistance(other.Tile, npc.Tile) >
                    Spec107.GroupHuntGatherRadiusTiles)
            {
                continue;
            }
        ...
        out NPCState stranger, out float hate)
    {
        ...
        if (gathered.Count < Spec107.GroupHuntMinGirls)
        {
            return false;
        }
        ...
        WorldState world, List<NPCState> gathered, NPCState stranger, out string blocked)
    {
        ...
        if (!Spec107.GroupHuntEnabled || stranger is null || stranger.Health <= 0f)
        {
            blocked = "NoStranger";
            return false;
        }
        ...
        foreach (var girl in gathered)
        {
            ...
            if (affinity > Spec107.GroupHuntHateThreshold)
            {
            ...
            if (HexSpatialMath.HexDistance(girl.Tile, stranger.Tile) >
                Spec107.GroupHuntMaxPactDistanceTiles)
            {
                blocked = "TooFar";
                return false;
            }
        ...
        WorldState world, List<NPCState> gathered, NPCState stranger)
    {
        ...
        foreach (var girl in gathered)
        {
            ...
            girl.Mind.GoalLock = new GoalLock
            {
                ...
                EndTick = world.Tick + Spec107.GroupHuntLockTicks
            };
