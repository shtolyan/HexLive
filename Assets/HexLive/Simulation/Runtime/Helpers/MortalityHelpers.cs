...
internal static class MortalityHelpers
{
    ...
    private static void Die(WorldState world, NPCState npc, DyingCause cause, string source = null)
    {
        ...
        // ⭐ баг #169: она УЖЕ ЛЕЖАЛА. Окно умирания (§105) — это лежачее время, и
        // смерть в его конце ничего не должна доигрывать.
        //
        // Метку ставим ЗДЕСЬ, потому что строкой ниже гаснет DyingCause, а
        // вместе с ним и IsDying — то есть и IsLyingDown. Свип трупов (MobSystem)
        // спрашивает именно его и на следующем Medium-проходе видел уже
        // «умерла стоя»: телу назначался обычный вариант смерти, и вид играл
        // клип из положения стоя. Со стороны это ровно то, на что жалуется
        // игрок, — «умер с голоду и зачем-то встал и лёг».
        npc.DeathAnimVariant = -1;
