...
// §70: how an outsider sizes up a kill. He is an OPPORTUNIST, not a berserker —
...
public static class RaidMath
{
    ...
    public static bool IsFitToRaid(NPCState npc, WorldState world)
    {
        if (npc.Health <= 0f ||
            ...
            npc.Health < Spec70.RaidSelfHealthFloor ||
            MobSystem.WorstPartHealth(npc) < Spec70.RaidSelfWorstPartFloor)
        {
            return false;
        }
    ...
    public static int AlliesAround(WorldState world, NPCState victim)
    {
        ...
        foreach (var other in world.Entities.Npcs.Values)
        {
            ...
            if (HexSpatialMath.HexDistance(other.Tile, victim.Tile) <= Spec70.RaidIsolationRadiusTiles)
            {
                count++;
            }
        }
    ...
    public static float Opportunity(WorldState world, NPCState raider, NPCState victim)
    {
        ...
        if (allies > Spec70.RaidMaxVictimAllies)
        {
            return 0f;
        }

        var isolation = Spec70.RaidCrowdCount > 0
            ? 1f - MathUtil.Clamp(allies / (float)Spec70.RaidCrowdCount, 0f, 1f)
            : 1f;
        ...
        var weakness = Spec70.RaidVictimHealthCeiling > 0f
            ? MathUtil.Clamp(
                (Spec70.RaidVictimHealthCeiling - condition) / Spec70.RaidVictimHealthCeiling, 0f, 1f)
            : 0f;
        var distance = HexSpatialMath.HexDistance(raider.Tile, victim.Tile);
        var proximity = Spec70.RaidScanRadiusTiles > 0
            ? 1f - MathUtil.Clamp(distance / (float)Spec70.RaidScanRadiusTiles, 0f, 1f)
            : 0f;

        return isolation * Spec70.RaidWeightIsolation +
            weakness * Spec70.RaidWeightWeakness +
            (helpless ? 1f : 0f) * Spec70.RaidWeightHelpless +
            proximity * Spec70.RaidWeightProximity;
    }
    ...
    public static NPCState BestVictim(WorldState world, NPCState raider, out float opportunity)
    {
        ...
        foreach (var candidate in world.Entities.Npcs.Values)
        {
            if (candidate.Health <= 0f ||
                ...
                HexSpatialMath.HexDistance(raider.Tile, candidate.Tile) > Spec70.RaidScanRadiusTiles)
            {
                continue;
            }

            // He gives up at the door, exactly like a dog (§29C.4A sanctuary).
            if (Spec70.RaidRespectsSanctuary && MobSystem.IsNpcInSanctuary(world, candidate))
            {
                continue;
            }
            ...
            if (score < Spec70.RaidOpportunityFloor)
            {
                continue;
            }
    ...
    // §70: the missing motive. Without this he is a hermit, not a hunter — his
    ...
    public static TileCoord? ProwlTarget(WorldState world, NPCState raider)
    {
        ...
        foreach (var pair in world.FactionHomes)
        {
            ...
            if (distance <= Spec70.ProwlArrivedTiles || distance >= bestDistance)
            {
                continue;
            }
