...
public sealed partial class ExecutionSystem
{
        ...
        WorldState world, NPCState npc, NPCState mark, bool submitted, string taken)
    {
        ...
        // ⭐ §107: СВИДЕТЕЛЬНИЦЫ. Без этого сцена портила отношения ровно с
        ...
        // ненависти взяться было неоткуда, и сговор §107 не складывался почти
        ...
        // Под общим выключателем §107: «выключил — поведение как до §107»
        // должно быть правдой целиком, а свидетельницы — его половина.
        if (Spec107.GroupHuntEnabled && Spec107.GroupHuntWitnessAffinityLoss > 0f)
        {
            foreach (var witness in world.Entities.Npcs.Values)
            {
                if (witness.Id.Equals(mark.Id) || witness.Id.Equals(npc.Id) ||
                    ...
                    HexSpatialMath.HexDistance(witness.Tile, mark.Tile) >
                        Spec107.GroupHuntWitnessRadiusTiles)
                {
                    continue;
                }
                ...
                seen.Affinity = MathUtil.Clamp(
                    seen.Affinity - Spec107.GroupHuntWitnessAffinityLoss, -1f, 1f);
                seen.Trust = MathUtil.Clamp(
                    seen.Trust - Spec107.GroupHuntWitnessAffinityLoss, -1f, 1f);
