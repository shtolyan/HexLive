...
public sealed partial class ExecutionSystem
{
    ...
    private static bool ShouldKeepSleeping(WorldState world, NPCState npc)
    {
        ...
        // ⭐ §118.8: раненая ОТЛЁЖИВАЕТСЯ — полная энергия не поднимает её,
        // пока восстановимое здоровье не дойдёт до WoundedRestExitHealth.
        // Проверка стоит ПОСЛЕ общего прерывателя выше, так что настоящий
        // голод/жажда и опасность будят её как всех.
        if (WoundedRestMath.ShouldKeepLying(npc))
        {
            return true;
        }
