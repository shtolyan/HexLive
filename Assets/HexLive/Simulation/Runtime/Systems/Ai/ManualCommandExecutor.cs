...
// §118: приказы игрока становятся планами. Единственное место, где команда
    ...
    private static void ApplyInteract(WorldState world, InteractCommand command)
    {
        ...
        if (standBeside)
        {
            ...
            if (!PlanningSystem.TryReserveBesideJunction(
                    world, npc, anchor, Spec118.ManualReserveTicks, out target,
        ...
        else
        {
            ...
            if (!SpatialMutations.TryReserveJunction(
                    world, target, npc.Id, world.Tick, Spec118.ManualReserveTicks))
            {
                Reject(world, npc.Id, "Interact", "Occupied");
                npc.Mind.CurrentGoal = GoalType.None;
                return;
