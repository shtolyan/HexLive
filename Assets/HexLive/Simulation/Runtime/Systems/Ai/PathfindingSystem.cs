...
public sealed class PathfindingSystem : ISimulationSystem
{
    ...
    // §70: the same soft ring, grown around hostile PEOPLE instead of mobs —
        ...
        foreach (var hostile in world.Entities.Npcs.Values)
        {
            ...
            while (_hostileQueue.Count > 0)
            {
                ...
                foreach (var neighborId in junction.Neighbors)
                {
                    ...
                    foreach (var tile in neighbor.Tiles)
                    {
                        if (HexSpatialMath.HexDistance(tile, hostile.Tile) <= Spec70.DangerRingTiles)
                        {
                            within = true;
                            break;
                        }
    ...
    // §70: the ring an NPC actually routes around — mobs plus hostile people.
    ...
    // §70: who walks around the outsider. Unlike a wolf ring this is NOT gated
    ...
    public void Run(WorldState world)
    {
        foreach (var npc in world.Entities.Npcs.Values)
        {
            ...
            // §70: …and around a hostile person, on the same soft terms.
