...
{
    // §70: how many of ONE faction are still alive. "The colony" used to be
    ...
    // what the pre-§70 `world.Entities.Npcs.Count` did (the death sweep removes
    ...
    // §70: this faction's camp anchor, or null if it has none authored.
    ...
    // §70: is this tile inside the given camp? A campfire or a bed has no
    ...
    // always true, which is why the pre-§70 answers are preserved exactly.
    public static bool InCamp(WorldState world, TileCoord tile, Agents.Faction faction)
    {
        ...
        return HexSpatialMath.HexDistance(tile, home) <= Spec70.MaxCampRadiusTiles;
    }
