...
// §107: форма расправы — кто с кем сцеплен, когда он бежит и чем всё кончилось.
...
public sealed class GroupHuntSystem : ISimulationSystem
{
    ...
    {
        if (!Spec107.GroupHuntEnabled || !Spec72.Enabled)
        {
            return;
        }
        ...
        foreach (var hunter in _hunters)
        {
            ...
            if (GroupHuntMath.PartyAlive(world, quarryId) < Spec107.GroupHuntMinRemaining)
            {
                FinishHunt(world, quarryId, done: false, reason: "PartyCollapsed");
                break;
            }

            // ⭐ ПОБИЛИ И ПРОГНАЛИ. С пощадой §107 он не падает — а расправе
            ...
            // пороге 6 — забой, а не взбучка (свалиться он с пощадой §107.6 не
            // может, так что сама собой драка не кончалась).
            var blows = PartyBlows(world, quarryId);
            if (blows >= Spec107.GroupHuntBlowsToRout)
            {
    ...
    private void ResolveQuarry(WorldState world)
    {
        ...
        foreach (var hunter in _hunters)
        {
            ...
            var stands = inReach <= 1 && quarry.Health >= Spec107.GroupHuntTargetFleeHealth;
            if (stands)
            {
        ...
        foreach (var hunter in party)
        {
            if (done)
            {
                ...
                hunter.Needs.Stress = MathUtil.Clamp01(
                    hunter.Needs.Stress - Spec107.GroupHuntStressRelief);
                foreach (var mate in party)
                {
                    ...
                    bond.Affinity = MathUtil.Clamp(
                        bond.Affinity + Spec107.GroupHuntBondAffinity, -1f, 1f);
                }
                ...
                if (quarry is not null)
                {
                    ...
                    grudge.Affinity = MathUtil.Clamp(
                        grudge.Affinity - Spec107.GroupHuntTargetGrudge, -1f, 1f);
                }
            }
    ...
    internal static void EndHunt(WorldState world, NPCState npc, string reason, bool announce = true)
    {
        ...
        npc.Mind.GroupHuntCooldownUntilTick = world.Tick + Spec107.GroupHuntCooldownTicks;
