...
public sealed class MobSystem : ISimulationSystem
{
    ...
    private void RunDog(WorldState world, Wildlife.MobState dog)
    {
        ...
        if (!inMelee)
        {
                ...
                HexSpatialMath.HexDistance(dog.Tile, target.Tile) <= 1)
            {
                ...
                if (!fledStandoff)
                {
                    ...
                    // §118: с приказом она не оборачивается и на разгон зверя —
        ...
        // §118: ручная не убегает от зверя сама — как и от человека (§109.8).
        ...
        //
        // ⭐ §118 ПРАВИЛО КЕНШИ, вторая половина: с приказом она НЕ встаёт в
