...
// §70: the shape of a raid — who is locked onto whom, who rallies, who runs,
...
// §70 adds is the attacker being a person, and knowing when he has had enough.
    ...
    public void Run(WorldState world)
    {
        ...
        foreach (var raider in world.Entities.Npcs.Values)
        {
            ...
            else
            {
                ...
                if (!fleeing &&
                    (victim.Health < Spec70.RaidVictimFleeHealth ||
            ...
            if (raider.Health < Spec70.RaidFleeHealth ||
                MobSystem.WorstPartHealth(raider) < 0.35f ||
                defenders >= Spec70.RaidBreakOffDefenders)
            {
                Trace.Emit(world, raider.Id, "RaidBrokeOff",
                    $"Health={raider.Health:F2} Defenders={defenders} " +
                    $"Reason={(defenders >= Spec70.RaidBreakOffDefenders ? "Outnumbered" : "Wounded")}");
