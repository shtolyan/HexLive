...
public sealed class PlayerVoiceCaptureRuntimeTests
{
    ...
    public IEnumerator SyntheticPcmDrivesCenteredOverlayAndDoneCancelButtons()
    {
        ...
        try
        {
            ...
            using (var submit = NavigationSubmitEvent.GetPooled()) root.Q<Button>(className: "agent-capture-done").SendEvent(submit);
            yield return null;
            ...
            Refresh();
            using (var submit = NavigationSubmitEvent.GetPooled()) root.Q<Button>(className: "agent-capture-cancel").SendEvent(submit);
            yield return null;
            Assert.That(cancelled, Is.True);
            Assert.That(capture.IsRecording, Is.False);
            Assert.That(root.Q("player-voice-capture").ClassListContains("agent-capture-visible"), Is.False);
        }
        finally { capture.Dispose(); Object.Destroy(host); Object.Destroy(settings); }
    }
