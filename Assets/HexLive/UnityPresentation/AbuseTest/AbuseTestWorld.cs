...
namespace HexLive.UnityPresentation.AbuseTest
{
    ...
    public static class AbuseTestWorld
    {
        ...
        public static WorldBootstrapDefinition Build(int seed = 313)
        {
            ...
            //
            // §81.11: здесь годами стоял «sit.stump» — id ИНТЕРАКЦИИ, а не
            // объекта. AddObject не валидирует id, объекты создавались молча,
            // Perception и рендер их не видели — арена, написанная ради
            // пенька, жила БЕЗ пеньков. Константа, чтобы это не повторилось.
            Put(ContentIds.PalmStump, -3, -1);
            Put(ContentIds.PalmStump, 2, 1);
            Put(ContentIds.PalmStump, 0, -2);
