...
public sealed class SimulationRunnerBehaviour : MonoBehaviour
{
    ...
    private static void RegisterDefaultSystems(SimulationEngine engine)
    {
        ...
        // §70: AFTER MobSystem — it clears IsFighting for every NPC each medium
        // pass, so anything that sets the latch has to run later.
        engine.Register(new RaidSystem()); // §70: the outsider's hunt and the colony's answer
        // §70: BEFORE AnimalCombatSystem — a body has one swing slot, and a man
        // with a knife outranks a dog for that tick of attention.
        engine.Register(new HumanCombatSystem()); // §70: timed human-vs-human blows
