...
namespace HexLive.UnityPresentation.Config
{
    ...
    public sealed class AttributesBalanceConfig : ScriptableObject
    {
        ...
        [Tooltip("Человеческое среднее. Тело ровно на нём даёт все множители = 1, то есть до-§76 игру. НЕ крути её, чтобы «дать очков»: она же служит точкой отсчёта эффекта, поэтому поднимается вместе с телом и не даёт ничего (доказуемый no-op). Прибавка — ниже.")]
        [Range(0.1f, 0.9f)] public float attributeMean = 0.5f;
        [Tooltip("§139.4: прибавка ко ВСЕМ шести врождённым характеристикам, +0.1 = +1 из десяти каждой (шесть очков россыпью). Сдвигает только ролл, точка отсчёта эффекта остаётся на среднем. Держи среднее + прибавка + разброс <= 1, иначе полоса клипается.")]
        [Range(0f, 0.4f)] public float attributeBonus = 0.1f;
        ...
        [Range(0f, 0.5f)] public float attributeSpread = 0.4f;
