...
namespace HexLive.UnityPresentation.Config
{
    ...
    public sealed class GearConfig : ScriptableObject
    {
        ...
        [Tooltip("§142: трусца с предметом — слот 0.5 блендера походки (§71). Пусто = обычная трусца. " +
                 "До §142 вооружённой была ТОЛЬКО ходьба, и с ускорением предмет терял стойку.")]
        public AnimationClip armedSlowRun;
        [Tooltip("§142: бег с предметом — слот 1 блендера походки (§71). Пусто = обычный бег.")]
        ...
        [Tooltip("Предмет несут ДВУМЯ руками. Верхний слой аниматора (маска «только руки») " +
                 ...
                 "клипа — поэтому одна поза покрывает стойку, шаг, трусцу и бег. " +
                 "Выкл = одна рука, как до §142.")]
        public bool twoHandedCarry;
        [Tooltip("Поза удержания (клип; берётся ОДИН кадр — см. время ниже). Пусто = первый клип " +
                 "атаки этим оружием: его стартовый кадр и есть авторская стойка «оружие наготове». " +
                 "У копья это Bayonet Stab — обе руки на древке.")]
        public AnimationClip carryPose;
        [Tooltip("Какой кадр клипа выше замирает как поза, долей длины (0 = первый).")]
        [Range(0f, 1f)] public float carryPoseTime;
        [Tooltip("Довести свободную кисть до древка через Final IK. Кисть проецируется на ОСЬ " +
                 "предмета и сдвигается на минимум — поэтому рука не выворачивается, даже если " +
                 "поза удержания авторски рассчитана на другую длину. Выкл = только поза.")]
        public bool offHandGripIk = true;
        [Tooltip("Где на длине предмета лежит вторая ладонь, долей от ведущей руки к острию. " +
                 "Работает как ЦЕЛЬ проекции: ближе к 0 — хват у самой руки, к 1 — у наконечника.")]
        [Range(0f, 1f)] public float offHandGripFraction = 0.3f;
