...
namespace HexLive.UnityPresentation.Config
{
    ...
    public sealed class HexTuningConfig : ScriptableObject
    {
        ...
        [Tooltip("Сколько держать шаг поверх остановки, когда сим говорит «маршрут ОКОНЧЕН» " +
                 "(прибыла/стоит). Короткая: длиннее — марш на месте после каждого прибытия. " +
                 "Разворот на месте исключён из держалок — см. ниже.")]
        [MirrorIgnore]
        [Range(0f, 1f)] public float walkHoldSeconds = 0.3f;
        [Tooltip("§71.8: держать шаг поверх заминки В ПУТИ (сим ещё ведёт её: MovementStatus=Moving, " +
                 "включая очередь у занятого джанкшена). Замер: заминки по 2-3 тика (0.5-0.75 с) — " +
                 "несколько раз в минуту ходьбы; 0.8 мостит их без Walk→Idle→Walk.")]
        [MirrorIgnore]
        [Range(0f, 2f)] public float midJourneyWalkHoldSeconds = 0.8f;
