...
namespace HexLive.UnityPresentation.Config
{
    ...
    public sealed class SocialBalanceConfig : ScriptableObject
    {
        ...
        [Tooltip("§49 r2: до какой энергии длится сон (0.999 = полная шкала с допуском float). Конец у сна ОДИН и по энергии: ни рассвет, ни мороз не будят — только настоящая нужда или опасность.")]
        ...
        [Range(0, 2400)] public int sleepDangerRecencyTicks = 600;
        ...
        [Tooltip("§53.8: сколько тиков живёт ПАМЯТЬ о страдающей, ушедшей из виду (час игрового времени — за него либо спасли, либо всё кончилось).")]
        [Range(60, 4800)] public int aidMemoryMaxAgeTicks = 600;
        [Tooltip("§53.8: какая доля заявки помощи остаётся у ВСПОМНЕННОЙ (не видимой сейчас) страдающей — живая перед глазами важнее, иначе колония ходила бы к призракам мимо живых.")]
        [Range(0f, 1f)] public float aidMemoryBidShare = 0.6f;
