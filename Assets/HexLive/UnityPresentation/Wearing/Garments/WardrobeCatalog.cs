...
public static class WardrobeCatalog
{
    ...
    private static List<string> Collect(string prefix, bool nested)
    {
        ...
        // ⚠️ ЗДЕСЬ НЕЛЬЗЯ ЖДАТЬ. Стояло WaitForCompletion на инициализации
        // Addressables — и вешало игру намертво: старт вызывается из корутины
        // экрана загрузки, а блокирующее ожидание внутри корутины не
        // разрешается никогда. Шторка не уничтожалась, мир оставался на паузе
        // (timeScale = 0), персонажи не появлялись вовсе. И ни одной ошибки в
        // логе — это не сбой, а тупик.
        //
        // Поэтому перечень СОБИРАЕТСЯ ЛЕНИВО, по первому спросу: к тому
        // моменту, когда он кому-то понадобится, Addressables давно поднят.
