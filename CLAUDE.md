...
### Граница разрешения на production

Настройка API-ключа, SSH, DNS, TLS, env-файла, reverse proxy или отдельной
службы — это **configuration-only** задача. Она не разрешает собирать или
разворачивать новый игровой сервер, переключать `/opt/hexlive/current`, менять
simdata/content/wire protocol либо собирать новый клиент. Даже root-доступ и
наличие более свежего commit не расширяют задачу. Минимальный restart допустим
только для службы, чью конфигурацию попросил изменить игрок.

Game-server deploy выполняется только после явной команды игрока обновить
конкретно названный VPS в текущей задаче. Разрешение для Сингапура не действует
на Нью-Йорк и наоборот; прежнее разрешение не переносится на следующие задачи.
Если configuration-only изменение невозможно без нового бинарника, остановиться
до production mutation, назвать версии и влияние на совместимость клиента и
попросить отдельное разрешение. Нельзя превращать настройку ключа в обязательную
рассылку нового клиента. Перед любой production mutation написать в commentary:
сервер, слой (`configuration`/`service`/`server binary`/`content`/`client`) и
...
> ⭐ **Баг-трекер — на СИНГАПУРЕ, и он никуда не переезжал** (указание игрока
> 2026-09-09). Всегда смотри баг-трекер и собирай билд, ОСНОВЫВАЯСЬ на
> Сингапуре: `https://vmi3529459.contaboserver.net/api/bugs/v1`. Нью-Йорк —
> дополнительный игровой сервер; у него свой отдельный список багов, и для
> релиза/снапшота он НЕ канонический. `Tools/build_release_windows.py` по
> умолчанию бьёт в Сингапур (переопределяется `HEXLIVE_BUG_API`). Клиентский
> game-endpoint (`ServerBook.ProductionUrl`) — отдельная штука и мог быть
> переключён на Нью-Йорк; это не меняет, где живут баги.
