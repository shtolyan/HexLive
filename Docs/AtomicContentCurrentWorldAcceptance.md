...
Дата проверки: 2026-08-27. Контур: изолированный staging
...
- Editor staging smoke: PASS — реальный remote bootstrap, 16
  репрезентативных payload, authored hanger из bundle шкафа,
  все модели текущего мира и owner-icons.
- Рабочий набор текущего snapshot: `84` model-payload — `actor=1`,
  `building=6`, `hair=3`, `mob=2`, `object=37`, `prosthetic=4`, `wear=31`.
- До снятия loading curtain готовы все 84 модели и 46 настоящих owner-icons.
...
- Вешалка/шкаф: `furniture.wardrobe@4` содержит нативную раму,
  рейки, полки, верёвочные соединения и скрытый authored
  `HangerTemplate`. Занятый слот клонирует его из того же bundle;
  `Cube`/`Sphere`/`Cylinder` meshes гейтом запрещены.
...
- `object/furniture.wardrobe`: `revision 3 → 4`; bundle включает
  authored `HangerTemplate`, SHA-256
  `5092af4d219de51c40120d55d74c4652fb8eac74b1a4e94fdb0bdf6126aa8ebd`,
  размер `203688`, внешних bundle dependencies `0`.

Повторная публикация первых четырёх кандидатов дала `0 changed`.
Публикация только шкафа изменила registry `4517 → 4518`; другие object
revisions и blobs не менялись.
