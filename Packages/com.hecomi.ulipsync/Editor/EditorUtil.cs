...
public static class EditorUtil
{
    ...
    // HexLive patch (spec §67.7): the microphone module is stripped from this
    // embedded copy — any Microphone reference forces a privacy usage string
    // into player builds, and the game never listens to a mic (lip sync is
    // fed from FMOD-played files). The selector stays as a stub for the
    // editor windows that still call it.
    public static void DrawMicSelector(ref int index)
    {
        index = EditorGUILayout.Popup("Device", index, new string[0]);
    }
