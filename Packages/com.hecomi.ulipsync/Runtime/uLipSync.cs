...
public class uLipSync : MonoBehaviour
{
    ...
    // HexLive patch (spec §67.7): Unity audio is disabled (FMOD owns
    // playback), so AudioSettings.outputSampleRate reports 0 and every
    // window/downsample ratio below would collapse. External feeders
    // (NpcVoiceLipSync) declare the true sample rate of the PCM they push
    // into OnDataReceived instead.
    public int overrideSampleRate = 0;
    int systemSampleRate => overrideSampleRate > 0
        ? overrideSampleRate
    ...
    int inputSampleCount
    {
        get
        {
            if (!profile) return systemSampleRate;
            float r = (float)systemSampleRate / profile.targetSampleRate;
    ...
    void ScheduleJob()
    {
        ...
        var lipSyncJob = new LipSyncJob()
        {
            input = _inputData,
            startIndex = index,
            outputSampleRate = systemSampleRate,
