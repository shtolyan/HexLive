...
public sealed class AgentDiagnostics
{
    ...
    public static string FailureKind(Exception error) => error is HttpRequestException { StatusCode: { } status }
        ? "Http" + ((int)status).ToString(System.Globalization.CultureInfo.InvariantCulture)
        : error.GetType().Name;
