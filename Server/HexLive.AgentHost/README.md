...
`on` копирует готовый runtime и запускатель на внутренний диск в
`~/.local/state/hexlive/agent-masha`. Это необходимо для launchd при хранении
проекта на ORICO: разрешения Terminal на внешний диск не наследуются фоновым
процессом. Память и секреты остаются по прежним защищённым путям.
