...
public sealed class McpEventLog
{
    ...
    /// <para>
    /// <c>ManualOrderRejected</c> и <c>ManualOrderFinished</c> идут через
    /// <c>Trace.Emit</c> и доступны без диагностики. Завершение нужно контроллеру,
    /// потому что sweep сразу очищает временный Plan.Status (§160, #362).
    /// <c>ManualOrderAccepted</c> остаётся диагностикой; синхронное подтверждение
    /// приёма агент получает непосредственно из результата MCP-команды.
    /// </para>
    /// </summary>
