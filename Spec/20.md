...
- Water renders sunken below its tile top. The sea/water tops use the
  hand-written **`HexLive/StylizedWater`** shader (Always Included Shaders, так
  что Player-у он доступен на первом кадре без бандлов и ленивых загрузок):
  вершинная волна с аналитической нормалью в пикселе, двухтоновый френель
  глубокое→мелкое, дрейфующие блики. (r2, 2026-08-27: раньше первым выбором
  значился импортный «Definitive Stylized Water URP», но с миграции на
  атомарный контент его ленивый `AtomicResources.Load` отдавал null на первом
  обращении, статический кэш это запоминал — и игра месяцами рисовала
  fallback; когда definitive однажды реально загрузился, игрок прочитал его
  как регресс — приподнятая волна, полосы у берега. Канонический вид воды —
  самописный шейдер; definitive остаётся референсом в
  `Assets/ThirdParty/StylizedWaterShader/`, вне Resources и вне сборки.)
  **Day/night:** the water shader ignores scene lighting (the sea glowed at
  night), so `CreateWaterMaterial` hands out a runtime material
  (`ActiveWaterMaterial`) and `SkyDayNightController` scales its colours with
  the directional light every frame — authored look at noon, deep moonlit
  blue at night.
