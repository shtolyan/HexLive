...
- v11 — CORRECT DIVE GEOMETRY (superseded in part by v13 above). A wall
  ...
  `HexTuningConfig` ScriptableObject (atomic `config/hextuningconfig`), applied
  on the loading screen before either the local or remote backend starts and
...
- ⭐ TWO SOURCES OF TRUTH, and the drift is silent. The game reads
  `HexTuningConfig` (atomic `config/hextuningconfig`); the **SwimTest scene reads its own serialized
...
- bug #251 — ATOMIC PRESENTATION TUNING IS PART OF BOOT. After the config asset
  moved out of `Resources`, the ordinary game stopped applying it altogether.
  Edge sitting therefore used the stale code fallback `+0.400 wu` instead of
  the authored `-0.344 wu`, putting the actor exactly `0.744 wu` too high (and
  `0.744 wu` too high again when sitting one elevation step up: `0.950` instead
  of `0.206`, because the shared step is `0.55 wu`). The loading screen now pins
  and applies `config/hextuningconfig` before selecting either backend, while
  the code and ScriptableObject fallbacks match the authored values
  (`LedgeSeatLift=-0.344`, `LedgeSeatBack=0.20`). Furniture is deliberately a
  separate coordinate contract: a stump seat stays at `ObjectGroundY` (the
  stump mesh is `0.30 wu` high), and a bed uses its explicit sleep point
  (`SleepRootLocalY=0.37 wu`; an integrated hut floor adds `0.107475 wu`).
