...
**Реализация (§167.4):** `DirectiveMath.Pull(kind, goal)` — параметр `command`
в `AddGoalScore`, последнее слагаемое `FinalScore`; `GoalScore.CommandModifier`
вернулся и печатается только когда не ноль. Источник — принятое указание
`NPCMind.Directive`, а не прямой приказ игрока: приказ (§121) остаётся
интерраптом, просьба — смещением.
...
An active plan is matching only when its action belongs to that plan. In
particular, `Sleep/InProgress` may not coexist with an active non-`Sleep` plan
or goal. Saves can preserve this contradiction indefinitely: execution keeps
the bed pose and occupancy alive while planning sees a valid work plan and
skips replanning. `SleepPlanConsistencySystem` runs before pathfinding,
movement and execution, aborts the stale sleep through the canonical 23.17
cleanup, returns the body to its retained wake junction, and leaves the desired
work goal intact for a clean replan. This is the bug #128 invariant; recovery
sleep without an active plan is intentionally outside it.
