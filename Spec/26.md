...
### 26.26 Сторож брони предметов (bug #382)

`IsOccupied` / `CurrentUser` — временное использование, а не вечное владение.
Перед восприятием каждого medium-такта общий сторож освобождает заявку, если
нет живого исполнителя, оставшегося шага активного плана или спасателя,
который ведёт указанного пациента в эту кровать. Подход спасателя **до взятия
пациента на руки** тоже держит кровать. Сон без обычного плана (обморок,
восстановление) держит её через активную Sleep-интеракцию.

Успешная долгая работа, путь, будущий шаг, материалы выкладки и незавершённый
крафт сохраняют бронь; произвольного TTL нет. Отмену зависшего пути выполняет
штатный timeout плана, после отмены бронь снимается. Прерывание освобождает все
заявки исполнителя сразу, включая вторичные предметы и уже failed/completed
исполнение. После загрузки сохранённые бесхозные заявки очищаются тем же сторожем.

Цена проверки зависит от NPC/оставшихся шагов и индекса заявок, а не от числа
предметов острова (§158). `EntityRepository.RegisterObject`, `UnregisterObject`
и `ClearObjects` обслуживают индекс на bootstrap/spawn/despawn/load; изменение
любого из двух occupancy-полей автоматически обновляет индекс. Сам индекс
не сериализуется и не меняет формат сохранения.

На трупах, останках и оторванных конечностях `CurrentUser` хранит происхождение
тела. Сторож никогда его не стирает; временное `IsOccupied` на таком объекте
живёт только пока есть фактическая интеракция. Постоянный `Owner` кровати и
предметов не изменяется.
