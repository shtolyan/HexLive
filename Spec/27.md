...
| Parameter | Value |
|---|---|
| Perception radius (objects) | hex distance <= 3 tiles |
...
| Seeded home knowledge | **own-camp objects only**, `IsPermanent`, never expires (r2) |
| Memory cap (r2) | `AiBalance.MaxKnownObjects` = 256 non-permanent records |

**27.18A r3 / bug #409: личная память обследования для ручного поиска.**
У ручной живой колонистки предметный сенсор запоминает собственные обследованные
тайлы и последний tick в пределах обычного `AiBalance.PerceptionRadiusTiles`.
Это не общая `world.ExploredTiles` и не сведения о скрытых предметах. Максимум
4096 тайлов на NPC; при заполнении вытесняется самый давний, равенство разрешают
Q/R. Сон/бессознательное состояние не добавляют обследование. Память сохраняется
в NPC-хвосте blob v77; v66–76 загружаются с пустой личной памятью обследования.
Явный Explore выбирает среди прежних допустимых целей максимум новых тайлов в
предметном радиусе будущей позиции. На стыке оценка берёт минимум новых тайлов
по всем возможным сторонам прибытия: округление мировой позиции не определяет
тайл сенсора после остановки. При равенстве — давно обследованную позицию,
затем прежний сидированный выбор. Оценка не читает содержимое скрытых тайлов.
Если все доступные области известны, допустима повторная проверка старой области;
нахождение конкретного предмета и безопасный сон не гарантируются. Автономный
Explore не использует новый приоритет и сохраняет прежний выбор.

**27.18A r2 (Aug-2026, решение игрока — «мир по памяти», BigIsland):**
с рождения колонистка знает только СВОЙ лагерь (очаг, кровати, запасы —
`IsPermanent`, в потолок не считаются и не вытесняются). Дикий мир — пальмы,
валуны, плавник — больше не сеется: его открывает восприятие §125, и он живёт
в памяти по обычному TTL. Поверх TTL стоит потолок: когда непостоянных записей
больше `MaxKnownObjects`, забываются самые давние по `LastSeenTick`
(тай-брейк — меньший id, ради детерминизма соаков). Мотивация двойная:
геймплей (разведка осмысленна, всезнание на 6× острове — карта из ~1100
объектов у каждой с тика 0) и производительность (`Perception.Objects` — это
память, и все предикатные сканы скоринга линейны по ней; потолок делает цену
тика ИИ независимой от размера острова).
...
- **Foraging (27.11):** when GetFood finds no known food item, the NPC walks
  to the nearest known producer whose `Produce` output is tagged `Food` and is
  usable with her current tools (e.g. a palm when she has a blade) — a move-only
  plan with no interaction. Arriving brings dropped fruit into perception
  radius. If already at the producer and no fruit is visible, this is fresh
  negative evidence: the producer is temporarily shunned for `ShunTicks`.
  Следующая попытка идёт к другой известной пальме; если таких не осталось,
  `GetFood`/`GetWater` уступает `Explore`, чтобы открыть новую территорию, а не
  повторяет пустой цикл ожидания у одного дерева до смерти.

`Produce != null` само по себе не означает еду: `forest.deadfall` производит
`resource.stick`. И доступность цели, и выбор маршрута используют одно правило
пригодности производителя, поэтому `GetFood` не может держаться доступной и
ходить к источнику древесины.
