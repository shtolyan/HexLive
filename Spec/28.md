...
- Authority → perceived hierarchy or obedience. **Реализована в §167.1**:
  растёт у слушательницы к просившей при принятом и выполненном указании,
  остывает как дружба; лидер — производная, а не поле.
...
| Вид | капсула-заглушка, затем ничего | её тело сразу в общей конечной позе `FallenIdle` → спрайт скелета той же длины и курса |
...
**Состояние тела — состояние мира, а не украшение кадра.**
`NPCState.DeathAnimVariant` по-прежнему выбирается СИМУЛЯЦИЕЙ по хешу сида, один раз, в
момент переезда, и едет в сейве (blob v20) и по проводу (wire v5, секция
`Corpses`). Это legacy-совместимое данное нужно в том числе для варианта будущих
останков, но вид свежего трупа больше не выбирает из него клип падения.

Если персонаж умирает уже лежащим и неподвижным, отрицательный вариант позы
сохраняет этот факт для симуляции/сейва. Однако вид для любого варианта сразу идёт
в общую конечную позу `FallenIdle`: ни нового крика, ни клипа падения труп не запускает.
...
метку сохраняет (`preserveLyingDeathPose || DeathAnimVariant < 0`). После загрузки
sentinel восстанавливает ту же детерминированную запись состояния; визуально
все варианты используют общую конечную позу `FallenIdle`.

Свежая смерть, загрузка сейва и позднее подключение зрителя визуально равны:
вид сразу просит общий `SetFallen` и принудительно ставит `FallenIdle` на его
конечный кадр. Переходные `death2`/`death3` не проигрываются — игрок не видит
повторного падения и ни на одном пути не получает промежуточную T-позу.

Аниматор выключается совсем (`enabled = false`, не `speed = 0`) **только после
подтверждения фактического state hash `FallenIdle`**. Запрошенный флаг лежания
не считается подтверждением. До этого единый actor render-gate скрывает только
каналы изображения, не меняя scale, кости и геометрию; тот же gate складывает
причины от NPC-impostor, ожидания позы и ожидания внешности через OR, поэтому
снятие одной причины не раскрывает тело вопреки другой.

Труп сохраняет последний честный облик NPC: надетую одежду, рюкзак и holster,
износ/грязь/кровь вещей, погоду кожи, раны, шрамы, повязки, ампутации и протезы.
Асинхронно приехавшая одежда, волосы и протезы присоединяются к тому же gate;
историческая покраска кожи и каждой вещи получает ограниченный приоритетный
первый composite (не больше одного painter за кадр), и только подтверждение
всех этих каналов раскрывает меши. Первый асинхронный miss карты покраски кожи
или вещи не считается отсутствием рисунка: он без фоновой трёхсекундной задержки
перепроверяется ограниченной полосой до загруженной карты либо явного terminal
missing/failed; для вещи до этого не подтверждается и входной hash. Ошибка GPU
composite также оставляет вещь неготовой для следующей ограниченной попытки.
Тот же gate хранит обязательные обмотки бинтом и марлей даже у чистого тела без
активного кровотечения: после `Ready` они размещаются до первого видимого кадра,
а явное terminal-отсутствие карты/арта получает невидимый tombstone и не держит
труп заблокированным бесконечно.
Сохранённый `WornBloodiness` восстанавливает детерминированный общий рисунок и
без текущей повреждённой зоны: заживление тела не отстирывает одежду и труп,
умерший позднее от другой причины, после загрузки не появляется в чистой вещи.
Seed рисунка складывается из ключа надетой вещи и стабильного `npcId`, а не из
Unity `InstanceID`, поэтому повторный stream-in не перебрасывает пятна.
Live-only реквизит в руке, одежда в руке,
полоса работы, речь и боевые флаги гасятся атомарно при переходе в труп, чтобы
не дублировать предмет из руки в holster. До выборки конечного кадра также
мгновенно обнуляются вес `ArmedCarry` и все живые IK/процедурные писатели костей:
верхний слой инструмента или взгляд не может остаться замороженным поверх
`FallenIdle`. До двух суток вид постепенно теряет
гигиену кожи, визуально темнея и покрываясь грязью по мере разложения. После
двух суток вид актёра уничтожается вместе с записью в `snapshot.Corpses`, и
цена останков ещё двое суток — один `SpriteRenderer` и один `WorldObjectState`
независимо от числа вещей.

**Опора трупа (bugs #45, #170, #354).** Труп использует ровно геометрию общего
`SetFallen`: корень actor-view ставится на `ActorGroundY` **текущего** тайла, а
`bodyRoot` держит общий ground-lying basis `localPosition = 0`,
`localRotation = identity`. Отдельного bounds-derived corpse-offset, второго
владельца Y или особого угла нет — после переноски, загрузки и свежей смерти
получается та же опора, что у живого персонажа в `FallenIdle`.
...
### 28.15G Запрошенная тема штатного разговора (§144, #359)

`TalkToCommand` может нести необязательную типизированную `RequestedTopic`.
Разрешены только общие темы: `SmallTalk`, `Escape`, `Dogs`, `Weather`, `Food`,
`Fire`, `Home`, `Gossip`, `Flirt`, `Joke`, `Grumble`. Личные жалобы о нуждах
и `Stranger` с возможным сговором не являются произвольными намерениями.
Явный whitelist не расширяется автоматически при добавлении enum-членов.

Приказ проходит прежние проверки управления, подход, приглашение, отказ
по прибытии и исход отношений. Тема сохраняется в `NPCPlanState.RequestedTalkTopic`
во время подхода и в save v73. При старте и обновлении темы разговора она
подменяет только общий случайный выбор; `PickSpeakerTopic` по-прежнему может
показать настоящую личную жалобу участницы. Отмена, завершение и следующий
приказ снимают запрошенную тему; обычный Talk без неё остаётся прежним.
Пузырь и готовая реплика Hexkufa используют штатный `CurrentTalkTopic` и
`SpeechCatalog`; отдельной TTS, принудительного исхода или изменения согласия нет.
