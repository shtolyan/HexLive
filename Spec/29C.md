...
| **Starvation / dehydration** | while Hunger >= 0.95 OR Thirst >= 0.95, every body part loses **-0.00036 per slow tick** (both at once: -0.0006); Health follows the body mean, a destroyed vital ends it (`StarvedToDeath` trace). Design anchor (Aug 2026): **two full days with not a single sip = 100→0** — thirst ramps 0→0.95 in ~238 slow ticks, the damage window is the remaining ~2762 of the 3000 slow ticks in two days; with both needs maxed the body gives out in ~1.3 days |
...
agent drains to death (now a matter of in-game days, not seconds — see the
two-day anchor above).
...
**Combat stand-off (`MobStats.MeleeHoldDistance`, wolf 0.9 wu, 0 = off).**
...
`woundStamps` + `maxWoundStamps`, `visualYawOffsetDegrees` (поворот только
тела относительно вектора движения). Краб имеет `footprintFraction=0.16`:
при `HexRadius=1.5 wu` его длина равна `0.24 wu`, ровно вдвое меньше прежних
`0.48 wu`; yaw `-90°` оставляет маршрут и root-forward прежними и разворачивает
тело глазами вперёд по ходу. Число подобрано игроком по факту (bug #245
rework: при `+90°` краб на глаз шёл «ровно задом», и от идеи боковой походки
игрок отказался — «пускай глазами вперёд»); авторская ось модели — данность,
а не константа, поэтому знак сверяется только в игре. A mob with no prefab path keeps its legacy
...
- **Food avoidance (iteration 11 hard lesson):** GetFood pickup targets and
  ...
  **Starving** may override stale danger memory (desperation is consistent
  with 23.17 emergencies), but never a live mob within those same 2 tiles:
  hunger can justify revisiting an old risk, not walking into a predator that
  is visibly camping the target. Without this, a dog pair camping a fruit tree
