...
приказы игрока, адресные факты контроллеру и причины смерти. Это часть игры, а не отладка: ими питаются
лента колонии, звук §67, речь, серверный канал и `DeathRecord.Cause`. Пишутся
ВСЕГДА, гейта не имеют.

Адресные `AgentObservedIntruder`, `AgentObservedTheft`, `AgentObservedLoot`
(§160.6) передаются только контроллеру наблюдавшего NPC и будят его даже при
выключенной диагностике. Они не входят в публичный список `GameEventTypes`:
вызов `Trace.Emit` здесь обеспечивает доставку факта, а не общую ленту колонии.
