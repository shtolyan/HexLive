...
**§31.10A — каблук не двигает фото-точку куклы.** `BodyBones` поднимает живой
hip на `HeelPose.lift`, чтобы каблук стоял подошвой на земле. Это временная
поправка живой позы, а не рост персонажа и не часть портретного кадрирования.
Клон куклы запоминает фактически применённый (с учётом плавного веса) подъём и
снимает его с скопированного hip **до** `Animator.Rebind` и фиксированной
studio-позы. Поэтому один актёр использует одну нейтральную фото-точку с любой
обувью: смена каблуков не поднимает фигуру к верхней кромке и не обрезает голову.
...
**§31.13C — игровая пауза замораживает и персонажный вид (bug #144).**
Пауза через панель скорости останавливает не только тики симуляции, но и общий
scaled-time презентации (`Time.timeScale = 0`). Поэтому все `Animator`, мимика
и процедурные позы персонажей остаются на точной текущей фазе. Снятие паузы
восстанавливает предыдущее положительное значение времени, а открытие и
закрытие Escape-меню не снимает паузу, поставленную игроком заранее.
