...
1. **Wear prefab (visual).** The prefab must contain: a root with the `Wear`
   ...
   This exception belongs only to that garment and that slot. Layer masks are
   composed from the inside out: an `Outerwear` exception never overrides a
   covering `Wear` garment. For example, an open jacket may leave `Belly` and
   `Pelvis` unmasked, but trousers on `Wear` still hide underwear there. After
   every equip or take-off the final outfit is recalculated across both higher
   clothing layers; the order in which garments were changed cannot alter the
   result.
   `Outerwear` has a second, independent authored list, `hideWearSlots`, for
   hiding a garment from the intermediate `Wear` layer. It is empty by
   default: jackets normally leave shirts and trousers visible. A listed slot
   hides the whole intersecting `Wear` garment (for example, `Chest` can hide
   a T-shirt beneath a closed jacket), without changing either garment's layer
   or unequipping the lower item. Bags never participate in this mask. Runtime
   actors and the inventory doll apply the same final layered composition.
   ...
   The open-strap Stars tactical vest (`clothing.vest_stars`) has the same
   authored `Chest` exception: on seed `-28260451`, NPC 1 equips it by tick
   2565 over her chest underwear. This exception belongs to that exact
   prototype, not to vests, armor or `Outerwear` as a family.
   ⭐ **ЮБКИ — исключение целым классом (баг #171).** По прямому указанию
   игрока все восемь юбок-прототипов (`clothing.skirt_*`, а с ними и все их
   цветовые вариации, которые носят арт прототипа) перечисляют в
   `noHideUnderwearSlots` ВСЕ свои слоты: под юбкой бельё видно всегда. Тем же
   заходом посадка каждой поднята на три шага степпера гардероба
   (`WearConfig.scale` 1.00 → 1.03, шаг степпера 0.01).
...
- The same selected-garment panel authors both visibility masks per occupied
  slot. `Бельё видно в слотах` edits `noHideUnderwearSlots`; for an
  `Outerwear` item, `Скрывает одежду в слотах` edits `hideWearSlots`. The
  latter starts empty, updates the live outfit through the normal redress path,
  is saved into the prefab, and is preserved by a forced re-extraction.
...
- `BikerPants` deliberately adds one normal WardrobeTest step (`+0.01`) to
  every shipped actress fit (bug #190): Molly/Jolly/Marta/Jana serialize
  `1.01 / 1.04 / 1.01 / 1.01`. The pre-existing Jolly difference remains;
  “all actresses” means an equal delta, not flattening their authored fits.
- After the player's fit check, `BikerPants` adds one more normal step for
  every actress (bug #194), for a total `+0.02` from the original values:
  Molly/Jolly/Marta/Jana serialize `1.02 / 1.05 / 1.02 / 1.02`.
