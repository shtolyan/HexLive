...
- **Produce rot** (29A, bug #411): an untouched whole coconut naturally
  dropped by a palm despawns after more than `WorldBalance.FruitRotTicks`
  (2400 ticks), on the next awake Slow tick. This cleans natural litter;
  gathered whole coconuts are a stock and do not expire under that cleanup.
  A fruit carries persisted `ProduceOrigin`: `Natural` for a palm drop,
  `Gathered` for a carried drop or a harvested branch yield. Felling the palm
  never changes its already dropped fruit's origin. Sleep defers the same
  age check until the fruit's chunk wakes. Pierced/open coconuts retain their
  existing ground lifetime regardless of origin; pickup/processing/drinking/
  eating keep their ordinary command and interaction paths.
  Blob v74 adds the origin byte to the object record. For v66–73, a saved
  `producer.ProducedItems` link proves `Natural`; other whole fruit stays
  `Unknown`, not falsely labelled gathered. Unknown old fruit is retained
  because its history cannot safely justify deletion. No prior removed stock
  is recreated, and no new storage UI or ownership restriction is introduced.
  `ProduceRotted` remains diagnostic; the fixed stock no longer disappears
  through that background cleanup.
...
- **Ledge sitting**: adjacent tiles differing by exactly 1 elevation form a
  ...
  prefers a valid ledge within 4 tiles. The view anchors that pose to the same
  snapshot `npc.Tile` from which `LedgeSeatStepsUp` was calculated; it never
  reverse-maps the exact seam junction through `WorldToTile`, whose boundary
  tie may belong to the lower neighbour (#346).
...
- **33.2 Cartoon rain** (`HexWorldRenderer.UpdateRain`): a world-space
  ...
  their actual hex surface and emit a small world-space splash there. For land
  the contact is `TileHeight + elevation*ElevationStep + 0.03`; with
  `TileHeight=0.20` and `ElevationStep=0.55` this is `0.78 wu` at elevation 1,
  `1.33 wu` at elevation 2 and `2.98 wu` at elevation 5. Water uses its
  shore-anchored still level (`0.20 + 0.55*(1-0.1) = 0.695 wu`) plus the same
  live `WaterWave.HeightNow` displacement as the mesh, then the common 0.03
  splash lift. The former single elevation-1 collision plane fixed bug #176
  but swallowed every splash on higher hills; bug #257 removes that accepted
  limitation without adding hundreds of tile colliders.
