...
- **Ledge sitting**: junctions on a clean boundary where adjacent tiles differ
  by exactly 1 elevation are scenic spots — sitting there means legs dangling
  over the edge. A vertex shared by three levels (`e0/e1/e2`) is not a seat:
  choosing one of its pairs left the actor on the lower side, half in the floor
  on the first attempt (#192). Ground-sit planning prefers a valid ledge within
  4 tiles.
