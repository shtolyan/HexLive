...
- **Exposure & sunburn** (needs at least one *uncovered* body part —
  ...
  - effective UV: **Roofed** or Water tile → 0; within 1 tile of a Shade
  ...
  - held redness stings (bug #283): while `Needs.Sunburn` > 0.3 (the
    `Sunburnt` chip threshold), Comfort -0.003 per slow tick with its own
    `Sunburnt` ledger row — independent of `SunExposure`, so the sting
    persists in shade until the red heals off (§40.7).
...
#### 35.4 r2 — крышу от санктуария отделили (баг #167)

Солнце перекрывает **перекрытие над головой**, а не флаг `Indoor`.

- `TileFlags.Indoor` значит «санктуарий»: тепловой бонус дома (+4°), точка,
  у которой собаки бросают погоню (§29C.4A/§72.12), запрет ставить туда
  постройки. Им размечены стартовый двор колонии (14 тайлов ручной разметки
  в `PrototypeWorldDefinitionFactory`) и стоянка чужака — **под открытым
  небом, без крыши и без стен в кадре**.
- `TileFlags.Roofed` значит ровно «сверху есть перекрытие». Ставится вместе
  с `HasFloor | Indoor` в `BuildingBootstrap.CompleteHut` (весь футпринт
  достроенного дома) и на достройке хижины в `ExecutionSystem.Build`.
- Читают `Roofed` только двое, и оба про солнце: `TemperatureSystem.EffectiveUv`
  (единственная формула эффективного УФ — панель персонажа зовёт её же) и
  карта теней в `EnvironmentSystem.RebuildShadows` (крыша всегда в тени и
  бросает виртуальный навес на соседей; санктуарий без крыши — не бросает).
- Симптом до правки: колонистка спала на кровати во дворе в полдень и
  получала ровный ноль УФ — ни загара, ни ожога, ни строки в панели.
- Сейв: блоб v49. В блобе v48 и старше флага нет, и крыша восстанавливается
  по настилу (`HasFloor`) — санктуарные тайлы настила не имеют и крыши не
  получают.
...
- **Anti-churn rework (spec 35.4a).** CoolOff used to be a *move-only* trip that
  ...
  1. **Overheat latch** (`IsOverheated`, enter `CoolOffEnterThreshold` 0.35 /
     ...
     `ThermalDiscomfort` is symmetric, therefore the latch may enter only when
     effective temperature is above 20°C and clears immediately on the cold
     side. A freezing survivor at discomfort 1.0 is not overheated; this also
     prevents cold `Dress` from repeatedly breaking an in-flight `Aid` plan.
  ...
  4. If two overheated actors see the last free cooling tile in the Decision
     pass, the first Planning pass may reserve it before the second. The second
     actor **defers** for the maximum dwell window instead of reporting a broken
     plan every generic 40-tick retry. This is cooling-capacity contention, not
     `NoCoolTile`; the next bid happens when the occupant can actually leave.
...
- **Rain effects**: effective UV ×0.5 (applied to `UvIndex` at source),
  global temperature −3° while raining. Rain wets every **exposed** wearable
  outdoors: worn items of NPCs on non-Indoor tiles and wearable ground objects
  on non-Indoor tiles (a coat dropped in the rain soaks — and so does one hung
  on the rack; nobody dries exposed laundry in a downpour).
- **Containers stop soak-through (r2):** одежда в `Inventory.Items` считается
  лежащей внутри производного контейнера §52 — рюкзака, кармана куртки или даже
  трусов. Дождь и Water-тайл мочат внешний надетый предмет, но не одежду внутри:
  рюкзак мокнет, сухая куртка в нём остаётся сухой; трусы мокнут, запиханная в
  них куртка — нет. Правило не привязано к `WearLayer.Bags` и работает для
  любого внешнего предмета-вместимости. Уже мокрая внутренняя одежда продолжает
  естественно сохнуть. Снятая вещь на земле остаётся внешней и промокает; её
  отдельные `WorldObjectState.Contents`, как и прежде, защищены оболочкой.
...
- **Body wetness** (`NPCState.BodyWetness 0..1`, MoistureSystem): the bare
  ...
  wetness reaches the garment materials. ⭐ Rain/water and SWEAT are two
  separate skin layers (bug #290): the rain pool dries to ZERO exactly like
  the cloth sheen, and sweat lives in its own layer with its old inertia
  (~12 s rise, same drain); the visible sheen is the max of the two. A
  single pool used to bottom out at the sweat level, so on a warm afternoon
  rain water stayed on the skin forever — clothes dried honestly while the
  body kept glistening «as if she just bathed», and got shinier as drying
  clothes returned their warmth.
...
- **Drying** (every slow tick an item is not being wetted, all locations —
  worn, carried, ground): clothing in ordinary world conditions dries at
  `Wetness -= 0.02 × 0.1`; direct sun gives ×3 of that natural rate (×0.3 of
  base). A lit campfire within 1 tile remains absolute ×4 and a drying rack
  absolute ×5 of base — bug #160 deliberately slows neither accelerated path.
  The wardrobe station likewise keeps its existing ×5 lit / ×1.5 unlit rate.
  Body wetness and non-clothing pickups retain the original natural ×1 / sun
  ×3 rates. The best applicable channel wins; multipliers do not stack.
