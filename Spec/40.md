...
- **Shipped v1:** stamina drains −0.05/slow-tick while working/moving,
  ...
  **Bug #353:** тот же динамический потолок ОСТАНАВЛИВАЛ отдых. В кровати
  запас рывком добегал до потолка (~0.60 при обычных голоде и комфорте) и
  замирал там на всю ночь — спящая просыпалась полувыжатой. Теперь **отдых
  восстанавливает запас полностью**: потолок отдыхающей (Sit/Sleep/Rest, а
  также кома и умирающая — те же ветки §60/§105) равен 1.0. Темп при этом
  прежний, `StaminaRestGain` за медленный тик, так что подъём плавный, без
  мгновенного скачка. Потолок бодрствования сохраняет свой смысл, но больше не
  срабатывает как обрыв: излишек над ним стекает по `StaminaIdleGain` за
  медленный тик — ровно с той скоростью, с какой простой его набирает, — так
  что после пробуждения нет snap-down, а голодная всё равно не остаётся бодрой
  надолго. Скорость стекания намеренно ВЫВЕДЕНА из набора на простое, а не
  заведена отдельной ручкой: две ручки тихо разъехались бы.
