...
- **§40.6 r7 — ОТКАЧЕНО (Sep 2026, эксперимент #338).** Правило ниже
  выключено целиком по решению игрока: проверяем, не оно ли даёт рывки на
  сервере. Запрет шов→шов снова строгий для всех; застрявшая на замурованном
  шве (кейс Ники) снова не выберется сама — спасать вручную/хирургией сейва,
  пока эксперимент не завершён.
- **§40.6 r7 (истор.) — «замурованный» шов скользит вдоль кромки (bug #334, Aug
