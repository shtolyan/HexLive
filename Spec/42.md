...
- **Sleep (superseded by §54.11 r2):** the old 100-tick interaction deltas
  were a second restore stream and are now zero. The single slow-tick clock
  fills 0→1 in **4 game hours on `bed.basic`** and **8 hours on the ground**;
  sleeping does not simultaneously pay the awake `EnergyRate` drain. Sleep
  (including exhaustion coma) is the **only** runtime source that increases
  Energy. Sitting on the ground, a ledge, a chair or `stump.palm` restores
  Comfort and Stamina but never Energy. Adrenaline keeps its alert/movement
  effects but no longer manufactures an Energy floor.
...
### 42.B MCP: согревание требует действующего огня (bug #416)

Видимый сейчас костёр в `visibleItems[].fire` сообщает `burning`, `canWarm`,
наличие топлива в буфере и рюкзаке, доступность инструмента розжига и
`ignitionPreconditionsMet`. Это предпосылки, а не гарантия подхода или занятости.
Ручной `Fuel` только кладёт топливо; затем нужен отдельный `Ignite` с инструментом
и лишь потом `Observe`. Навык `warm-by-fire` строит этот сегмент и проверяет
тепловое состояние после него. Память о костре не раскрывает его текущее пламя.
Ручной приказ согреться у погасшего костра отклоняется с `FireNotLit`; исполнение
повторяет проверку после подхода и при погасании, останавливая очередь.
