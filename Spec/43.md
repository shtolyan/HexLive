...
virtual steps at their first junction anchor — the same world-space anchor from
which Unity draws the mesh — and always shade their declared OWN tile (canopy).
Casting them from the tile centre instead displaced a junction-planted palm's
simulation shadow from its rendered shadow (bug #207). The
...
exactly like the drawn shadow), low canopies
(tent) default to 2; **Roofed** tiles block as +2 (the hut wall) and are
always shaded themselves — §35.4 r2: перекрытие, а не `Indoor`, у которого
второе значение «санктуарий» и который стоит на открытом дворе. A stump
