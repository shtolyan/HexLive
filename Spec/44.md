...
- **Presentation (shipped)**: `herb.bush` renders as a procedural low-poly
  ...
  Шесть известных зон тела имеют обязательные атомарные карты обмотки и
  рельефа. Пока реестр или одна из двух текстур ещё загружается, painter не
  создаёт stamp-key и повторяет размещение; старый круглый bandage не является
  fallback для limb-wrap и не может закрепиться на всю сессию из-за раннего
  асинхронного промаха.
