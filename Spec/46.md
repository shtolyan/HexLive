...
- **Уход видит игрок (Aug 2026).** `MobLeft` — строка хроники, парная к
  `NightRaid`: стая пришла, стая ушла. Без неё давление спадало необъяснимо.
  В `GameEventTypes`, `GameHistoryFormatter` (системное событие, без актёра —
  как `DogKilled`/`MobTookLimb`), термин `history.MobLeft` EN+RU, тон
  нейтральный. `MobLeaving` осталось ОТЛАДКОЙ (`Trace.DebugSystem` под
  гейтом): это не уход, а запись планировщика «зверь сверх потолка, уйдёт на
  тике N», она срабатывает пачкой на всех лишних сразу и в ленте была бы шумом.
