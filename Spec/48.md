...
must always be answerable by reading her status chips and the affected parameter's
hover card — never buried in a bespoke per-tick branch. The catalog is the **single ledger** of everything that acts on a
...
- **Surface (mandatory, always).** The effect exists in `EffectCatalog` with
  polarity, category, an icon and `effect.<kind>.title/.desc` localization. A
  status is emitted by `EffectEvaluator`; a parameter modifier is recorded by
  `EffectImpactLedger` at the point that actually applies it. Required for EVERY
  effect.
...
3. Surface it under the exact condition it acts: classify a status in
   `EffectEvaluator.Collect`, or record a parameter influence at its real mutation
   site through `EffectImpactLedger` (both when it is both a status and a modifier).
...
### §48.7 Parameter hover — the live influence ledger

The character card answers the narrower question “what is moving **this** bar
right now?” when the pointer rests on any need/parameter cell, including the
bipolar temperature cell. The card lists both helpful and harmful influences as
effect icons: harmful first with a red down arrow, helpful second with a green up
arrow. If nothing currently moves the parameter, it says so explicitly instead of
inventing a cause from the current value.

This is not re-derived in presentation. `NPCState.EffectImpacts` is a transient,
non-persisted `EffectImpactLedger`. A system that applies an ongoing parameter
change records, in the same branch, `(NeedKind, EffectKind, Positive|Negative)`.
Fast influences are cleared/re-written by the fast owner (movement/execution);
slow influences are cleared by `NeedsDecaySystem` and then extended by the later
`TemperatureSystem`. Recording has no numeric or ordering effect on the simulation;
it only describes the already-applied branch. Requested direction is recorded even
when the value is clamped at 0 or 1, so dirty clothes still explain zero Comfort
instead of disappearing precisely at the floor.

`NpcSnapshot.EffectImpacts` exports stable rows
`"NeedKind\tEffectKind\tPositive|Negative"` in the Character wire group. No live
float rate travels: the player needs the cause and direction, while omitting the
changing magnitude prevents the Character delta group from becoming a per-frame
bandwidth stream. The view resolves the icon/title through the same `EffectCatalog`
used by status chips. Thus `DirtyClothes` is a real Comfort effect; `Soaked`,
`Sick` and `Sunstroke` also point at Comfort when their actual branches apply;
`Cozy` points at Comfort positively. Hot/cold do **not** pretend to drain Comfort:
temperature owns the bipolar Temperature parameter (and severe extremes damage the
body), so their influence appears on Temperature, not on Comfort.

Загрязнение надетой одежды влияет на Comfort четырьмя диапазонами по среднему
загрязнению комплекта (грязь и кровь считаются общей загрязнённостью):

- `0–10%` — одежда свежая и даёт небольшой положительный эффект `CleanClothes`;
- `10–20%` — одежда считается чистой и на Comfort не влияет;
- `20–60%` — штраф плавно возрастает от нуля до прежней силы;
- `60–100%` — действует прежняя формула полного штрафа
  `dirtiness × DirtyClothingComfortLoss`.

`DirtyClothes` записывается в live-ledger только выше 20%, поэтому остаток после
стирки больше не подписывается как «Грязная одежда» (август 2026).

The first migrated ongoing sources cover every parameter shown in the needs grid:
natural metabolism/decay; sleep, bed and fireside recovery; wet and dirty clothing;
sickness; nearby company; witnessed suffering; work/rest; danger, body crisis,
calm, friendly conversation and crying; water/rain washing; bleeding/blood
recovery; ambient thermal drift; sprinting and breath recovery. Timed interaction
deltas (food, drink, sleep and comfort effects) use the same fast ledger while the
interaction is actively applying them. Instant historical events are not retained
after their mutation tick: the hover is “acting now”, not a combat log.

### §48.8 Общий поток чтения UI и MCP (#393)

`EffectReadModel` собирает статусы тем же `EffectEvaluator`, с единым контекстом
effective UV / fire warmth / sleep target. Snapshot передаёт уже вычисленный UV.
Generic struct sink принимает результаты напрямую; прежний List API остаётся
совместимым. В exporter больше нет промежуточного `List<ActiveEffect>` на NPC.

Влияния берутся только из текущего `EffectImpactLedger`: одинаковая тройка
need/kind/direction, записанная fast и slow владельцами, выдаётся один раз.
Противоположные направления сохраняются отдельно; величины не суммируются и
не пересчитываются. Ни classifier, ни reader не меняют ledger/needs/body.
Новый tick-проход, per-NPC cache и история эффектов не создаются.

`describe_colonist` (§144.14) получает оба слоя через этот же reader. Данные
описывают выбранное доступное тело; private effects соседних видимых людей
не расширяют §144.12. Snapshot wire-строки и UI sorting сохраняются.
