...
плана. У `tool.bottle` ограничения нет: каждая бутылка — отдельный
`ItemInstance`, а её `WaterKind/ResourceAmount` хранят вид воды и число глотков.
Поэтому несколько бутылок с разным содержимым честно занимают разные ячейки,
подбираются с земли и переезжают внутри bundle рюкзака. Припаркованная под
коллектором бутылка остаётся тем же физическим предметом в мире.
...
**Подбор при заполненном инвентаре (#378).** Доступность обычного `PickUp`
проверяется тем же `CanMakeRoomForGoal` перед началом действия, а реальная
вместимость — повторно перед переносом вещи: карман мог заполниться по дороге
или во время анимации. Свободное место внутри существующей стопки считается
местом; полезная разрешённая замена сохраняется. Если вещь не помещается,
попытка заканчивается неуспехом, освобождает владения и очищает действие/цель;
`Plan=Active` вместе с `Execution=Completed` после отказа недопустимы. Старое
сохранение с таким зависшим подбором тоже выходит из него при проверке
вместимости. Ручная очередь «собрать всё» завершается неуспехом, автономный
аукцион не обещает недоступный подбор и снова допускает его после освобождения
места. Сам предмет из-за полного кармана не получает временный `shun`.
Подбор мяса с вертела, извлечение содержимого тайника и немедленное надевание
результата крафта сохраняют собственные проверки фактически переносимой вещи.
...
(seed 1104049673). A bottle's importance follows its physical instance too:
its charges and provenance live in `ItemInstance.ResourceAmount/WaterKind`.
...
| `BottleCapacity` | 1 (implicit) | **10** | one litre = ten 100 ml gulps (§55.4) |

**Refillable bottle** — filling sets the selected physical bottle's
`ResourceAmount = SimBalance.BottleCapacity`; each `DrinkBottle` spends one,
and the bottle only empties (and warrants a refill trip) when the last charge is
gone. `ItemInstance.WaterKind` and `ResourceAmount` are one invariant: either
both describe a positive amount, or both normalize to `None/0`. Blob v68 and
snapshot wire v39 persist this pair per item. Blob v66/v67 migrates the former
NPC-global `BottleWater/BottleCharges` pair into its first bottle on load.
...
**A wardrobe bid needs a usable approach now** (§24.16). All ordinary Dress
probes (warmth, armor, covering and a missing backpack) share the planner's
exact approach check, including live bodies and other NPCs' reservations.
A visible garment with no currently usable stand point must not interrupt an
active meal route, fail planning and win again after its failure cooldown.
This is live availability, not a shun: freeing the stand point immediately
makes the garment eligible again. Need scores, fit, ownership and benefit
...
### §52.10 Отказ размещения не уничтожает владельца вещи (#396)

Успешный ground spawn предшествует удалению точного ItemInstance из инвентаря.
DropOrRetain сохраняет тот же экземпляр при отказе; аварийное снятие/прерывание
также сохраняет содержимое карманов, износ, запас, загрязнение и владение.
Возникший excess может временно превысить slot/carry-limit, но не теряется и
переживает save/load. Автоматический повтор ограничен transient
NextGroundDropRetryTick (не чаще slow interval); после загрузки допустима одна
новая попытка. Просто полный, но не переполненный pack ничего не выбрасывает.

Harvest/split/bath/dress recovery и разделка тела используют тот же путь
приёмки результата. Уже выданные yields не повторяются из-за отсутствия места:
разделка завершается, тело удаляется, оставшаяся экипировка сохраняется у
разделывающего NPC. Не допускается повтор всей транзакции после частичного
сброса вещей или уже выданного мяса/шкуры.

Завершение обычного раздевания при отказе размещения оставляет снятую вещь в
durable inventory и прерывает попытку с cooldown без успешного завершения.
Bathe/Laundry восстанавливает тот же worn instance и прерывает
текущий план с cooldown, сохраняя personal-care transaction; повтор strip каждый
тик недопустим. StowClothes сначала сохраняет вещь и все карманы в inventory,
потом очищает transient hand и завершает попытку отказом; ClothesStowed success
при таком отказе не публикуется.

Предпроверка замены одежды рассчитывает итоговые capacity/holsters и проходит
вытесненные экземпляры в исходном порядке WornItems. Свободное место принимает
вещь в pack; ground admission требуется только для фактического остатка.
Перелив, помещённый внутрь упавшей одежды, не занимает отдельные ground points;
финальный SpillOverflow резервируется отдельно в порядке исполнения. Dry-run
использует общую политику favorite/holster/importance и не меняет исходные
списки либо поля ItemInstance. При неожиданном runtime отказе каждый следующий
шаг учитывает реальную ёмкость после retention, а не устаревший список решений.
