...
`BottleWater` and `BottleCharges` are one invariant: either both describe a
positive amount, or both normalize to `None/0`. Blob v65 persists the charge
count; an older blob cannot recover it and therefore loads the bottle empty
instead of fabricating water (§41.2, §55.4).
