...
couldn't crawl to food, or one bleeding out beside a healthy neighbour, got no
help. §53 makes people **care for each other**: a girl who is herself okay
walks over to the worst-off permitted recipient and helps — feeds, dresses a
wound, hands a pill, or sits with the grieving — which bonds them both. In the
six-camp modes permission and urgency are scaled by the directed friendship
formula in §146.12 instead of requiring shared camp membership. All knobs live in the
...
The perception build tags each permitted `PerceivedAgent` with a `Suffering`
(0..1) and the single most-urgent **helpable** `AidKind`, in urgency order: **Treat** (open
...
For a different friendly camp the raw severity is multiplied by directed care
willingness (§146.12). A neutral non-critical neighbour therefore does not
silently consume supplies, a stronger friendship produces a stronger Aid bid,
and a dying neutral neighbour remains visible at the humanitarian `0.75`
minimum. Cross-boundary aid to/from `Outsiders` is always absent; outsiders may
still tend their own faction as before.
...
Revision 6: a ward whose authoritative `BedSleep` target is an occupied bed is
approached through the bed's free furniture rim, not through the five
body-local lying stations. Those stations are valid standing points around a
body on the ground, but on a bed they lie over the mattress and are rejected by
the bed's own solid footprint. Scoring and reservation enumerate the same rim,
discard walls, cliffs, third objects, live occupants and points beyond
`InteractionReach.Aid`, and choose the shortest reachable candidate. The helper
remains on that junction and faces the ward; she never snaps onto or walks
through the mattress. A bed against a wall therefore remains aidable from any
genuinely open side and unavailable only when its entire perimeter is blocked.
...
| Bandage / medicine | take a perceived ready `item.bandage` first; otherwise `CraftBandage` if she already carries 2 leaves, else `GatherHerb` |
...
pouch). The errand uses the possibility half only. A dressing still has to reach
her pack before it can be handed over, but `CraftBandage` now treats a perceived
ready dressing on the ground or in reachable dropped clothing as the first
supply step and builds a physical pickup plan instead of manufacturing a copy.
