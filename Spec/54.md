...
Одежда занимает отдельную точку, capacity=1. У stackable-предмета capacity
равна InventoryState.StackSizeFor, но каждый экземпляр сохраняет свой ObjectId,
владение, износ, загрязнение, влажность и запас. Совместимость стопки определяется
только definitionId, как у inventory slots: различающиеся состояния остаются
