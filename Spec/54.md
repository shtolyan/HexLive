...
- **`HarvestDrop` / `Yields`** on `InteractionDefinition` (`Definitions.cs`): a
  ...
  drops, places separate world instances through the shared local allocator
  around the source. A same-definition pile shares a junction up to its inventory
  slot capacity; other piles require separate non-overlapping footprints (§54.20).
  If no ground position fits, the exact output is retained rather than lost.
  One handler serves palm→logs, log→sticks, carcass→meat+hide. A Process removes
  its unconditionally consumed source before placing outputs, freeing that point
  without ignoring any other obstacle or repeating already-issued yields.
...
- **`resource.stick`** — the common fuel/craft currency. Fire, axe, pickaxe, spear,
  ...
  `Heartwood` material family defines the wood style used by world props. Since
  §151.2 the fire itself accepts any item tagged `Wood`: a log burns as four
  sticks, while a board or other wood burns as one.
...
A manual `Build` on an already fully stocked furniture site returns
`MissingTool` when its required hammer is neither carried nor available at the
site under the existing native finishing rule. It does not start a no-op build
animation. Delivery to an unfinished bill remains possible without a hammer;
hand-built products keep their existing exemption.
...
#### §54.2a r3 — порог «сначала собери, что лежит» обязателен для ВСЕХ веток валки (баг #340)

Прод-сейв HugeIsland (seed 527670712, tick 572000): 283 пня, ~10 000 лежащих
листьев — и порог 8 не остановил валку ни разу. Порог стоял только на листовой
ветке (`pendingLeafSource`); строительная ветка §146.8 (брёвна/доски под билл
пяти недостроенных домов лагерей, 193 пальмы > ценза 30) проверяла лишь «нет ли
бревна под рукой», роняя крону и листья как мусор, а топливная ветка порога не
имела вопреки тексту r2 выше. Теперь `looseHarvestBacklog <
SimBalance.LooseHarvestBacklog` — общий предикат топливной и строительной веток
(листовая уже держала его в `pendingLeafSource`). Затор рассасывается сам:
кроны съедает `ChopCrown` (крыши/кровати), листья вянут (§54.2c, баг #338),
после чего валка под стройку возвращается — но только на прибранный остров.
...
0.30-wu height and 0.63-wu trunk diameter. Its FBX export bakes the
Blender-to-Unity space transform into the mesh: the imported owner root keeps
an identity basis, so Unity X/Z remain the horizontal trunk axes. The common
`StumpFactory` 0.85 X/Z narrowing therefore produces a 0.5355-wu maximum world
diameter without reducing the 0.30-wu height; asset-specific runtime rotations
or axis swaps are forbidden. The two self-contained felled-crown
...
### §54.2c Пень и отрастание рощи (#315)

Свалить пальму — не «удалить дерево навсегда». Валка (`ExecutionSystem`,
завершение `Harvest` у объекта с тегом `Palm`) деспавнит пальму и на том же
тайле/фрагменте/джанкшене ставит **`stump.palm`**: низкое препятствие
(`ObstacleRadius` = 0.255 R = 0.3825 wu — на 15% уже ствола) с единственным
взаимодействием `sit.stump`. Тени пень не даёт (нет тега `Shade`), выкорчевать
его нечем.

**Пень зарастает обратно в ОБЫЧНУЮ пальму.** `FruitProductionSystem` (слой
`Slow`) меняет `stump.palm` на `tree.palm`, когда одновременно: пень не
`IsOccupied` (на нём никто не сидит — иначе вырастет следующим Slow-тиком),
у него `SpawnTick > 0` (то есть он срублен по ходу игры, а не поставлен
генератором мира), его якорный джанкшен ещё существует, и с `SpawnTick`
прошло больше `WorldBalance.StumpRegrowTicks` — **480 000 тиков = 20 визуальных
суток** (`DayLengthTicks` = 24 000). Отсчёт живёт в `SpawnTick` и переживает
сейв/загрузку. Проверка стоит ДО дневного гейта `cyclePhase`, поэтому пни
зарастают круглосуточно, в отличие от производства кокосов. В трейс уходит
`PalmRegrown`.

**Промежуточной «молодой» пальмы нет.** `tree.palm_small` (2 бревна вместо 3,
крона `resource.palm_crown_small`, `ShadeSteps` 5) выпилен из игры: он был
односторонним переходом — обратно в полноценную пальму дерево не росло, и
каждая срубленная большая пальма навсегда обедняла рощу. Стадий роста у дерева
не бывает: пень стоит свой срок и сменяется взрослой пальмой целиком. Id
retired: спавнить его нельзя, живой константы `ContentIds` под него нет, а
объекты из старых сейвов `WorldSaveSerializer.MigrateRetiredContent`
переименовывает в `tree.palm` (радиус блокировки у обоих одинаковый, так что
топология не меняется и дерево не исчезает из мира). Определение
`resource.palm_crown_small` остаётся в каталоге только ради таких сейвов — с
дерева эта крона больше не падает.
...
- **Early raise.** A `build.site` with `BuildProduct="campfire.spot"` becomes a
  ...
  block warmth. While the bill is open, interaction lookup MUST keep the real
  `campfire.spot` verbs (`Fuel`, `Ignite`, craft, warmth) rather than replacing
  them with the generic `build.site` definition; `build.furniture` on the
  campfire definition remains the separate upgrade verb.
...
### §54.20 Свободные предметы: реальные экземпляры и измеренные стопки (#396)

`GroundPileCatalog` — единый engine-free источник геометрии для симуляции и
`HexWorldRenderer`. Каталог неизменяем после инициализации. При сбросе ищется
ближайший к мировой позиции NPC разрешённый junction текущего тайла и кольца
соседей; собственная точка допускается и побеждает, если свободна её площадь.
Кандидаты упорядочены по квадрату расстояния, затем JunctionId. Площадь полной
стопки проверяется по локальному ObjectsByTile, с запасом по максимальным
измеренным радиусам соседних профилей; полного обхода объектов мира нет.

Одежда занимает отдельную точку, capacity=1. После запроса #418 ёмкость наземной
укладки задаётся отдельно в GroundPileCatalog и не выводится из правил инвентаря.
Инструменты (кроме бутылки) лежат рядами по 4, ткань — четырьмя стопками по 5
на одной точке (20 экземпляров, как и раньше),
таблетки/бинты/шины — плоскими рядами по 10. Остальные ранее поддержанные
ресурсы сохраняют 20, листья — 60, мясо и половинки кокоса — 8. Каждый экземпляр сохраняет свой ObjectId,
владение, износ, загрязнение, влажность и запас. Совместимость стопки определяется
только definitionId: различающиеся состояния остаются
на отдельных world objects и не сливаются. Контейнеры и одежда имеют capacity=1.
Целый и проколотый кокос имеют capacity=1, открытые половинки — 8. Исключены
слоты сушилки, гардероба и водосборника. PickUp у очага означает снять мясо со
вертела, а не переносимый очаг.

Выбор количества выбрасываемых экземпляров остаётся единственным действующим
механизмом §123.5. Весь инвентарный стек ткани из 20 экземпляров одной командой
размещается в одной наземной группе; прежние группы из сейвов не становятся
переполненными из-за нового вида. Нестаки инвентаря по-прежнему
выбираются по экземпляру, но совместимые инструменты после отдельных сбросов
могут лежать в одном наземном ряду. Содержимое вещей и количество никогда не
сокращаются для красоты. При нехватке места сохраняется атомарность всего сброса.

Палки: 20 экземпляров, пять столбцов и четыре шахматных слоя; полный габарит
1.05×0.473 wu по XZ и около 0.349 wu по высоте. Листья: 60 одинаково направленных
слоёв без yaw/jitter, шаг 0.001 wu; CPU-проверка пересечений треугольников
подтверждает разделение одинаковых XY-проекций. Полная высота около 0.21748 wu,
а не 60 высот изогнутого AABB. Половинки кокоса: 4×2, footprint0.75×0.37 wu.
Остальные generic-fit предметы используют консервативный центрированный
габарит sqrt(3)×TargetWorldSize: каждая исходная ось после ObjectFit не больше
TargetWorldSize, ортогональная ground pose укладывается в этот предел.
Для инструментов, ткани и лекарств #418 использует измеренные после штатного
FitObjectPrefab габариты (Tools/Art/GroundRows418.json), с наружным запасом
0.00001 wu. Инструменты располагаются вдоль более короткой горизонтальной оси
с зазором 0.01 wu. Поворот лежащего инструмента умножается на импортированный
базис, а не стирает его: зажигалка, мачете и пила больше не стоят вертикально.
Ткань: 2×2 стопки, шаг слоя — измеренная высота +0.002 wu; остальные новые ряды
имеют зазор 0.01 wu. Фактические полные размеры XYZ: четыре копья
0.302859×0.047592×1.350000 wu, 20 тканей 0.658020×0.664904×0.479083 wu,
10 упаковок таблеток 0.750060×0.043548×0.351011 wu. Модели, количество и
содержимое экземпляра не заменяются. Схема Docs/GroundRows418.svg построена из
фактических Renderer.bounds, сетка 0.375 wu; новые измерения воспроизводятся
Tools/Art/MeasureGroundRows.cs.txt в idle EditMode под MCP lease. Это проверка
исходных моделей и штатной посадки, не проверка доставки новых AssetBundles.
Для одежды измеряются реальные GarmentDropFactory branches, включая варианты
плоской/объёмной позы; профиль содержит их union. Для протезов используется
максимальная длина source vertex относительно сустава с реальным actor/bone
масштабом — инвариантная к L/R и вращению граница. Кроны обходят ObjectFit и
используют отдельные реальные PalmCrownFactory bounds.

Source grounding выполняется один раз внутри visual wrapper. Только внешний
GroundPileSlot получает slot offset; sim anchor, навигация и interaction target
остаются на junction. Лист центрируется по фактическим fitted renderer bounds
в XZ, без предположения о знаке импортированного pivot; высота и source rotation
не меняются. Перенос/отражение XZ не меняет доказанный зазор параллельных слоёв.
Rank сортируется по ObjectId и обновляется после подбора,
удаления и восстановления residency. Старые смешанные либо переполненные точки
не мигрируют и не теряют предметов: их экземпляры остаются в ограниченной
базовой позе без slot offsets. Новое размещение на такие точки запрещено.

Неизвестная геометрия — диагностический UnknownGroundGeometry:<id>, а не
выдуманный радиус; источник/ownership сохраняется (§52.10). Authoring input
Tools/Art/GroundPileGarments.json используется только генератором, не runtime.
`python Tools/author_ground_piles.py --check` проверяет source/meta SHA и точное
совпадение generated garment block; `--leaf-proof` воспроизводит mesh proof.
Safe Unity fixture Tools/Art/MeasureGarmentGroundBounds.cs.txt запускается только
владельцем MCP lease в idle EditMode. Raw full-sweep orphan failures сохраняются
отдельно от успешного required active/save-supported coverage; extractor не
переписывает raw failure в успех. Схема: Docs/GroundPileLayout.svg.
