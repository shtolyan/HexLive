...
### §57.10 Early cries and bounded defence (14 Aug 2026)

If an actor has already chosen `Flee`, the first landed bite emits the help cry;
waiting for several wounds made collective defence arrive only after collapse.
A defender who is already on station owns an active bounded `Wait` step instead
of a completed empty plan. It holds until the assist lock/exchange resolves,
then clears the assist and returns to the auction.
