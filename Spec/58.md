...
`LocalizationCoverageContractTests` перечисляет `GoalType` напрямую и требует
для каждого непустые EN/RU строки; новые цели нельзя добавить без перевода.
Тот же gate требует `effect.<kind>.title` и `.desc` для каждого `EffectKind`:
статус ампутации, травмы или потребности не может показывать сырой id.

### §58.6 Native macOS privacy prompt

The OS displays microphone permission before any game UI can render it. Its
`agent.voice.microphone_usage` term is authored once in
`_ArtSource/Voice/native_permissions.tsv`, imported into I2Languages.asset with
`Tools/i2_add_terms.py`, and exported by the macOS packaging step into
NSMicrophoneUsageDescription plus EN/RU InfoPlist.strings before signing.
The native prompt follows the OS language, not the game's F2 setting; it adds
no C# string table and does not replace FMOD capture (§160.5).

### §58.7 Имена в headless MCP

`Tools/export_npc_names.py` механически экспортирует только `npc.<id>.name`
из I2Languages.asset в `Server/HexLive.Server/Mcp/npc-names.json`. Это
генерированные данные, не второй авторский словарь: менять имя нужно в I2,
затем повторить экспорт; `--check` проверяет совпадение в исходном checkout,
а серверный regression gate — наличие встроенных EN/RU имён без Unity-ассета.
Экспорт отказывает, если порядок языков I2 отличается от `en`, `ru`.
JSON встраивается ресурсом в сервер и доступен без Unity или
репозитория рядом. Пустой русский перевод получает тот же EN fallback, что
`Loc.NpcName`; неизвестный или авторский id показывается самим собой.

MCP отдаёт `nameId` и `names` с EN/RU сразу: так внешний агент сопоставляет
произнесённую игроком «Дашу» с тем же именем в интерфейсе, независимо от языка
текущего диалога. Переводы никогда не прописываются C#-таблицей.

### §58.8 Эффекты в headless MCP (#393)

`Tools/export_effect_terms.py` экспортирует из I2 только title/description
видов `EffectCatalog`, cause-specific ключи classifier и названия `NeedKind`.
Генерированный `Mcp/effect-terms.json` встроен в сервер; `--check` проверяет
его свежесть. Единственный авторский источник остаётся I2; порядок en/ru
проверяется, пустой русский перевод получает EN fallback. Ответ MCP содержит
только определения видов и ключи, использованные в текущем ответе.
