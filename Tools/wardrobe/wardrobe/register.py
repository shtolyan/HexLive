...
def library_row(sim: dict, garment_id: str, layer: str) -> str:
    sex = sim.get("sex")
    if sex not in ("Female", "Male", "Any"):
        raise ValueError(f"{garment_id}: sim.sex must explicitly state Female, Male or Any")
    ...
    row = (f'                new("{garment_id}", "{sim["displayName"]}", '
           ...
           f'{sim["thermalDelta"]:.2f}f, dress, {sim["capacity"]}, GarmentSex.{sex}, {covers})')
