...
Любимое оружие занимает существующий визуальный слот на верхней части спины
(тот же, где раньше показывалось только копьё) и не выбирается жертвой при
освобождении переполненного инвентаря. Пока это один суперслот: отдельные
крепления для ножа и инструментов не вводятся.
...
Экспортёр также нормализует quaternion-трансформ всех `Window_window_rail`:
длинная ось рейки всегда вертикальна ещё в художественном master-файле, а Unity
не содержит поштучных компенсирующих поворотов.
...
Художественная поза по умолчанию открыта на `72°` относительно стены, а
...
- **Клик по человеку или зверю** — меню «Атаковать / Выбрать». Атака только
...
- Подсветка при наведении — своя, простая (подмешивание цвета в базовый через
  `MaterialPropertyBlock`), за интерфейсом `IHoverHighlighter`: купленный
  аутлайнер встанет на её место одной строкой.

### §121.5 Наведение: во что целится курсор

Курсор ищет цель тремя способами, по убыванию точности.

- **Скрытая листва не ловится ВООБЩЕ.** §112 убирает пальму из кадра ровно
  затем, чтобы игрок добрался до того, что под ней; ловящая курсор невидимая
  крона отменяла бы весь смысл. Сама пальма об этом не сообщает: прячут её
  переводом в `ShadowsOnly`, рендерер остаётся включённым — поэтому у
  `FoliageOccluder` появилось свойство `Hidden`.
- **Есть `ObstacleRadius` — целимся в вертикальный столб этого радиуса вокруг
  подошвы**, а не в габариты меша. ⭐ Иначе пальма перехватывает всё: в
  `palm_final.glb` ОДИН узел и один рендерер (ствол и крона — подмеши), его
  коробка 4.0 × 3.9 × 4.1 wu, то есть ШИРЕ гекса, при настоящем стволе 0.45.
  Камера смотрит сверху и входит в «потолок» этой коробки за метры до кокоса
  на песке, а кокосы падают в 0.9–2.6 wu от ствола — то есть заведомо внутри.
  Приём не новый: `CameraFoliageCuller` уже судит по стволу, а не по кроне.
- **Иначе — габариты рендереров.** Рендереры частиц исключены: у ПОГАСШЕГО
  костра они остаются включёнными и ловили бы пустоту над кострищем.
- Совсем без геометрии (якоря тела, могилы, воды, пустой стройплощадки) —
  маленький столб у подошвы. Иначе такие объекты нельзя навести в принципе.

Кэш рендереров пересобирается, когда вид меняет детей: куча на стройплощадке и
мясо на вертеле уничтожают и создают их заново на каждой доставке, и
одноразовый кэш делал такой объект ненаводимым навсегда. У срубленного дерева
маркер снимается: вид живёт ещё пару секунд, а номер объекта уже протух.

### §121.6 Подсветка обязана быть ВИДНА

Подмешивается ЦВЕТ, а не белизна. Первая версия осветляла базовый цвет к
белому — и не подсвечивала НИЧЕГО из текстурной графики: у одежды, камней,
палок, кокосов и пальм `_BaseColor` и так белый, блок записывал то, что там
уже стояло. Светились только объекты с кодовым цветом. Игрок видел это как
«подсвечивается не всё», и причина была не в пикинге.

Блок пишется по индексу материала, а не на рендерер целиком, и перекрашивается
каждый кадр, пока наведено: `GarmentWorldCondition` переписывает одежде свой
блок каждый тик, и подсветка иначе жила бы один кадр из шестнадцати.
Подсвечен всегда ровно один объект, поэтому перекраска бесплатна.

### §121.7 Чужая собственность

Как в Kenshi: чужое видно ДО клика, а не выясняется отказом.

- **Спать в чужой кровати нельзя.** Наведённая чужая вещь светится другим,
  холодным цветом; в меню заголовок называет владельца, а пункт «Спать» серый
  с пояснением, чья это кровать.
- Правило ОДНО — `ColonyQueries.ForbiddenOwner` — и читателей у него два:
  исполнитель приказа (авторитет) и меню, которое серит пункт заранее. Список
  глаголов, которые владение запрещает всегда, тоже один
  (`OwnershipAlwaysBlocks`). Две копии предиката — ровно тот класс бага, ради
  которого написан `AvailabilityMirrorsPlannerTests`.
- Фляга в водосборе спрашивает СВОЁ, уже написанное правило
  (`WaterCollectorMath.CanTake`): оно не сводится к «чужое значит нельзя» —
  со своей флягой в руках колонистка просто переливает, не трогая чужую.
  Поэтому меню её пункт не серит: врать про законное действие хуже, чем
  показать отказ на месте.
- Умер хозяин — вещь ничья, и правило переживает это само: уборщик владения
  ходит редко, а игрок кликает сразу.
- ⚠️ **Спрашивает это только ПРИКАЗ ИГРОКА.** ИИ живёт как жил: §64 оставил
  владение кроватью мягким предпочтением намеренно (`SpecDream.BedExclusive` —
  «меняет баланс выживания»), и строгий режим здесь не включается. Поэтому
  поведение мира не меняется и golden trace остаётся чистым.
- Владелец едет в снапшот отдельным полем `OwnedByNpcId`. Переиспользовать
  `OwnerNpcId` НЕЛЬЗЯ: там `CurrentUser`, то есть «кто пользуется сейчас / чьё
  это тело». Их уже однажды перепутали — инспектор гекса до сих пор
  подписывает занятость словом «Owner».
