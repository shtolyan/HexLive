...
30 самостоятельных «кубиков»: 6 опорных вершин, 6 частей пола, 9 сплошных
стеновых пролётов, 2 оконных, 1 дверной и 6 треугольных roof-patch. У каждого
...
порядок. Каноническая готовность каждого кубика хранится в
`WorldObjectState.ArchitectureElements`: стабильная идентичность
`(building ObjectId, ElementId)`, definition, slot, локальный transform,
поштучно доставленные ресурсы, работа и доступность. Save blob v32 и snapshot
wire v16 перевозят этот список без повторного распределения после загрузки.
...
самодельной геометрии хижины нет. Модель уже авторится в мировом масштабе и
дополнительно в Unity не масштабируется; её измеренный габарит — примерно
`0.766 × 0.34 × 1.60 wu`. Длинная ось каждой кровати идёт вдоль локального
`Z` здания. Симуляционный anchor остаётся на безопасном внутреннем junction
`local X=±0.6495 wu`, но визуальная кровать и поза сна сдвигаются наружу на
`0.246 wu`: центр оказывается на `local X=±0.8955`, а внешний край сохраняет
лишь технический шов около `0.0205 wu` до стены. Поэтому мебель вплотную занимает
краевое пространство, не превращая пристенный узел в доступную NPC-позицию.
Нативный FBX
содержит только 69 видимых строительных деталей, поэтому `BedAssembly` добавляет
штатный дочерний маркер `point` на мировой вертикали настроенной высоты `0.24 wu`
...
гекса `X=±1.299038`, между его вершинами `Z=±0.75`. Реальная рама меньше —
примерно `0.57 × 1.42 wu`, то есть оставляет около `0.04 wu` художественного
воздуха от стен и соседних предметов. Все 14 junction, попавшие в логический
