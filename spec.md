...
- **SHIPPED (40.8-J — бесшовные декали: рисунок продолжается ЧЕРЕЗ шов):**
  ...
  - Проверка без Play: **HexLive ▸ Paint Maps ▸ Probe Skin Seams** —
    печатает покрытие каждой группы и на скольких ячейках каждой зоны бинт
    реально задевает больше одного слота (это и есть места, которые раньше
    резались пополам).
- **SHIPPED (40.8-H — zone-damage speckles, чисто косметика):** per-zone HP
  ...
  Альфа 0.45→1; размер и количество ПЕРЕМНОЖАЮТСЯ (урок r5: кривая под
...
{PopIcon, SpeechId, Tone}`. Ключ с суффиксом падает на `"<base>:*"`, потом на
голый `<base>` — новый мобId рисует осмысленный бабл в день, когда его завели,
а старые снапшоты играются как раньше.

**107.3 Всё над головой — в белом бабле.** Кьюшки летели мимо системы баблов:
голая эмодзи без подложки, взлёт-затухание 1.25 с, без рейт-лимита — самые
частые (`TalkRequest`/`TalkIncoming`) читались как строб, а не как речь. Теперь
у кьюшки СВОЙ белый бабл (тот же спрайт, 0.52 против 0.70 юнита), тот же
разворот `SmoothStep` и холд 1.7 с; новая кьюшка поверх показанной подменяет
содержимое НА МЕСТЕ и рестартит холд, а не разворачивается второй раз. Рядом с
занятым разговорным баблом кьюшка отходит влево. Внутри — эмодзи (тонируется по
`CueTone`) или круглый портрет собеседника (никогда не тонируется).

Попы отношений «+/−» намеренно оставлены как есть: это не реплика, а индикатор
результата, как всплывающий урон.

Через `NpcSpeechDirector` кьюшки НЕ маршрутизируются: его 6-секундный
`GlobalGap` глотал бы их. Рейт-лимит кьюшек — дедуп по `SocialCueTick` в симе.
...
- **Взгляд в камеру.** `NpcActorView.BeginPortraitGaze(eye)` / `EndPortraitGaze()`
  ...
  `clampWeightEyes 0.2`, `bodyWeight 0`, а **`headWeight` намеренно мал (0.35)**
  — камера висит на осях кости головы, и сильный поворот гнался бы сам за собой.
  `NpcFaceAnimator.SetEyesHold(true)` глушит моргание на те же кадры.
- **Съёмка — стейт-машина**, а не два кадра: `Aim` (навести и ЗАМОРОЗИТЬ камеру,
  включить взгляд) → `Converge` (4 кадра, IK доезжает) → `Rendering` → `Capture`.
  Взгляд снимается на КАЖДОМ выходе, включая отказной, иначе колонистка так и
  останется смотреть в точку, где висела камера.
...
- **Раз в сутки, днём.** Окно 10:00–16:00 (`timeOfDay01 ∈ [4/24, 10/24]`,
  0 = 06:00), по одному NPC с интервалом 0.75 реальной секунды, `_bakedDay`
  против повторов. Ночной кадр — тёмный силуэт, и фотографией не был.
  `RequestNow` (первая встреча) не изменён: тёмный снимок заменится в ближайшее
  окно.
