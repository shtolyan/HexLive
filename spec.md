...
**72.9 Отгружено ВЫКЛЮЧЕННЫМ — что проверено и что нет.** `Spec72.Enabled` и
...
модель, сейв v17, раздвоение восприятия, все гейты «свой/чужой», его лагерь и
...
НЕ доведено: **налёт не запускается сам**. Сначала он был ставкой в аукционе и
не выигрывал ни разу за 10 дней — измерено: у одиночки `Sit` доходит до 0.85,
а `DecisionSystem` переигрывает аукцион ТОЛЬКО между взаимодействиями
(`DecisionSystem:118`), так что короткое окно «она одна» почти не совпадает с
переторговкой. Поэтому старт переписан на ПРЕРЫВАНИЕ — той же формы, что клич
§57 и первый удар §62: `RaidSystem.TryStartHunts` рвёт текущий план и берёт цель
напрямую.

Но и прерывание в соаке молчит, и причина пока не найдена. Что при этом
установлено измерением, чтобы следующий заход не начинал с нуля: жертва,
проходящая порог, доступна **65 замеров из 600** после грейса; `Spec72.Enabled`
в прогоне ИСТИНА (это доказывает `HostileSpotted=247` — тот же флаг гейтит
хостильный скан §62); `RaidSystem` зарегистрирован; движок исключения не
глотает (их нет вовсе); отладочный трейс, поставленный первой строкой цикла,
в исполняемой DLL присутствует — и всё равно не появляется в событиях.
Следующий шаг — искать именно здесь, а не в весах и не в гейтах.

Пока это не сомкнётся, включать нельзя: колония не получает ни одного налёта.
