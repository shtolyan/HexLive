...
## §107 Групповая охота — сговор против чужака (iteration 107)
...
Проактивной мести в игре не было вовсе. §107 даёт её — но не как решение одной
...
кончается вся линия §81. Это принятая цена, см. §107.6.
...
**107.1 Свидетельницы — недостающее звено.** Первая версия почти никогда не
...
**107.2 Тема разговора — человек.** `TalkTopic.Stranger`, дописана в конец enum.
...
**107.3 Сговор — единогласно.** `GroupHuntMath.TryFormPact` при общем розыгрыше
...
**107.4 Держатся вместе — задний шлюз, без лидера.** `BuildGroupHuntPlan`:
...
**107.5 Расправа.** `GroupHuntSystem` (Medium, зарегистрирована МЕЖДУ
...
**107.6 Исход решает лестница ненависти, а не §107.** `Spec107.
...
**107.7 Концовки и расплата.** Удалась → стресс каждой −`GroupHuntStressRelief
...
**107.8 Карве-ауты у соседей.** `ThreatAlertSystem` не гнёт охотнице маршрут
...
§107 — единственное санкционированное исключение из «девушки не начинают
...
**107.9 Ручки** (`Spec107`, зеркало в `OutsiderBalance.asset` + `simdata.json`):
`GroupHuntEnabled` (true — выключено = поведение до-§107 БАЙТ В БАЙТ, проверено:
...
**107.10 Замеры** (прототипный остров, 24000 тиков = 10 дней, шесть сидов).
...
кончается `Routed`. Смертельные исходы бывают и это ожидаемо (§107.6).
...
**107.11 Что осталось.** Свои реплики на хекскуфе (сейчас тема берёт
