...
## §70 Враг-человек — фракции, охотник и сплочённый отпор (iteration 70)
...
**70.1 Данные.** `Agents/Faction.cs`: `enum Faction { Colony = 0, Outsiders = 1 }`
...
слой схлопывается в до-§70 поведение без второй ветки кода.

**70.2 Свой и чужой.** Чоук-пойнт — восприятие. `PerceptionSystem` строил
...
**70.3 Его лагерь.** Спавн вычисляемый, не хардкод: рельеф — шум от сида,
...
**70.4 Охота.** Новая цель `GoalType.Raid`, дописанная **в конец** enum'а.
...
**70.5 Бой — по таймингам §29C.3, и это не стилистика.** Очевидное решение
...
**70.6 Оборона.** `ThreatAlertSystem` смотрит и на враждебных людей: ⚠️ за
...
**70.7 Ручки.** `Runtime/Balance/Spec70.cs`, зеркало —
...
**70.8 Презентация.** Он выбирается и инспектируется как все, но выжившим
колонистом не считается. Кьюшка сигнала — существующая `DangerSpotted` →
`Warning.png`, так что новых спрайтов и терминов не нужно.

**70.9 Отгружено ВЫКЛЮЧЕННЫМ — что проверено и что нет.** `Spec70.Enabled` и
`SpawnOutsider` по умолчанию `false`, как §40.17 отгружался «измеренно спящим».

Проверено пробой (headless, simdata, сиды 12345/777/999 × 10 дней): фракционная
модель, сейв v16, раздвоение восприятия, все гейты «свой/чужой», его лагерь и
второй очаг, ⚠️ и обход, и — на снятых самогейтах — **вся боевая цепочка
целиком**: подкрадывание 23 → планов 20 → сцепок 4 → его ударов 16 → **ответных
ударов девушек 17** → отходов 3 → они его убили. То есть охота, бой, сплочённый
отпор, отступление и смерть работают ровно так, как задумано.

НЕ доведено: при отгружаемых значениях ручек цель `Raid` **не выигрывает
аукцион**. Причины измерены: у одиночки `Sit` доходит до 0.85 (он постоянно
некомфортен и без сил), а `DecisionSystem` переигрывает аукцион только между
взаимодействиями (`DecisionSystem:118`), так что редкие окна доступности почти
не совпадают с переторговкой. При `RaidBaseScore = 0.9` цепочка запускается.
Оставить включённым в этом состоянии нельзя: колония теряет выживших (3/12
против 5/12) и не получает взамен ни одного налёта. Следующий шаг — подобрать
вес на 1–3 налёта за 10 дней без просадки выживаемости и снять флаг.
